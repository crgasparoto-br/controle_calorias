ALTER TABLE `whatsappConnections`
  ADD COLUMN `activePhoneKey` varchar(32) NULL AFTER `phoneNumber`;
...
SET connection_row.`status` = 'disabled',
    connection_row.`activePhoneKey` = NULL,
    connection_row.`updatedAt` = NOW()
WHERE ranked_active.active_rank > 1;
--> statement-breakpoint
UPDATE `whatsappConnections`
SET `activePhoneKey` = CASE
  WHEN `status` = 'active' THEN `phoneNumber`
  ELSE NULL
END;
--> statement-breakpoint
