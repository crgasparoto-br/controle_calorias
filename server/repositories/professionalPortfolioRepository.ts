...
function mapItem(row: Row): ProfessionalPortfolioItem {
  return {
    ...
    nextReviewAt: null,
    pendingItems: String(row.authorizationStatus) === "pending" ? 1 : 0,
  };
}
...
export function createProfessionalPortfolioRepository(
  ...
  async function list(
    ...
    const inactiveBefore = new Date(Date.now() - 3 * 86_400_000);
    ...
    try {
      ...
      const [itemsResult, countResult, summaryResult] = await Promise.all([
        db.execute(sql`
          SELECT a.id AS authorizationId, a.patientUserId, u.name AS patientName,
            ...
            t.status AS trackingStatus, a.requestedAt,
            m.lastFoodActivityAt, h.lastProfessionalInteractionAt
          ${baseFrom}
      ...
      return {
        items: rowsFromResult(itemsResult).map(mapItem),
        pagination: {
          page: input.page,
