...
type SanitizedHabit = NonNullable<ReturnType<typeof sanitizeHabit>>;
type HabitSnapshotRow = SanitizedHabit & { userId: number };

function buildHabitSnapshot(userId: number, habits: HabitMemoryState[]) {
  const byFoodName = new Map<string, HabitSnapshotRow>();
