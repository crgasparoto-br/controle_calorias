...
describe("professional record service", () => {
  ...
  it("paginates the timeline independently from other record collections", async () => {
    const execute = vi
      .fn()
    ...
    const result = await getProfessionalRecord(10, {
      patientId: 20,
      page: 1,
      pageSize: 20,
    });
    ...
    expect(result.guidances[0]).toMatchObject({
      visibility: "patient",
      authorName: "Nutricionista",
    });
  ...
  it("blocks every new clinical intervention while tracking is paused", async () => {
    getDb.mockResolvedValue({
      execute: vi.fn().mockResolvedValue([
        scopeRow({ trackingStatus: "paused" }),
      ]),
    });
    await expect(
      createProfessionalNote(10, { patientId: 20, content: "nota" })
    ).rejects.toThrow(
  ...
  it("creates a new immutable assessment version and audit event", async () => {
    ...
    getDb.mockResolvedValue({
      ...
      transaction: (callback: (tx: { execute: typeof txExecute }) => unknown) =>
    ...
    ).resolves.toEqual({ id: expect.any(String), nextReviewAt: null });
