...
describe("standaloneCommandWords", () => {
  it.each(["registrar", "CONFIRMAR", "cancelar", "editar", "consultar", "sim", "não", "ok", "2", "opção 3"])(
    "bloqueia a mensagem inteira %s",
    value => {
      expect(isStandaloneWhatsappCommandWord(value)).toBe(true);
    },
  );

  it.each(["registrar 100 g de arroz", "adicionar 1 banana", "consultar registros de ontem"])(
    "não bloqueia comando completo %s",
    value => {
