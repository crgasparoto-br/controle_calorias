...
describe("simulateWhatsappInbound", () => {
  beforeEach(() => {
    ...
    processMealDraftMock.mockResolvedValue({
      draftId: "draft-1",
      processed: { items: [{ foodName: "pão de cenoura", canonicalName: "pão de cenoura" }] },
      media: [],
    });
  });
  ...
  it("bloqueia ajuste numerico sem contexto antes do fallback nutricional", async () => {
    ...
    expect(result).toEqual(expect.objectContaining({ handled: true, action: "router_safe_response", reply: expect.stringContaining("em qual item") }));
  });

  it("responde sugestao sem acionar LLM, texto ou parser nutricional", async () => {
    const result = await simulateWhatsappInbound(42, { text: "sugira um jantar leve" });
    expect(executeWhatsappLlmIntentMock).not.toHaveBeenCalled();
    expect(executeWhatsappTextIntentMock).not.toHaveBeenCalled();
    expect(processMealDraftMock).not.toHaveBeenCalled();
    expect(result).toEqual(expect.objectContaining({ handled: true, action: "router_safe_response", data: expect.objectContaining({ canonicalIntent: "sugestao_refeicao", shouldAllowNutritionFallback: false }) }));
  });

  it("responde urgencia de saude sem acionar LLM, texto ou parser nutricional", async () => {
    const result = await simulateWhatsappInbound(42, { text: "estou passando mal e com falta de ar" });
    expect(executeWhatsappLlmIntentMock).not.toHaveBeenCalled();
    expect(executeWhatsappTextIntentMock).not.toHaveBeenCalled();
    expect(processMealDraftMock).not.toHaveBeenCalled();
    expect(result).toEqual(expect.objectContaining({ handled: true, action: "router_safe_response", reply: expect.stringContaining("Procure um serviço de urgência"), data: expect.objectContaining({ canonicalIntent: "possivel_urgencia_saude", shouldAllowNutritionFallback: false }) }));
  });
  ...
  it("pede esclarecimento para dia da semana ambiguo antes de alterar dados", async () => {
    const result = await simulateWhatsappInbound(4211, { text: "almoço de sábado", receivedAt: new Date("2026-06-15T12:00:00.000Z"), userTimezone: "America/Sao_Paulo", messageId: "time-2" });
    expect(processMealDraftMock).not.toHaveBeenCalled();
    expect(result).toEqual(expect.objectContaining({ handled: true, action: "temporal_context_clarification_needed", data: expect.objectContaining({ temporalExpression: "sabado", ambiguityReason: "dia da semana sem passado ou proximo" }) }));
  });

  it("registra fallback de fuso quando usuario ainda nao tem timezone configurado", async () => {
    const result = await simulateWhatsappInbound(4212, { text: "café da manhã hoje cedo: banana", receivedAt: new Date("2026-06-15T12:00:00.000Z"), messageId: "time-3" });
    expect(logInferenceEventMock).toHaveBeenCalledWith(expect.objectContaining({ userId: 4212, status: "warning", eventType: "whatsapp.time.temporal_context_resolved" }));
    expect(result).toEqual(expect.objectContaining({ temporalContext: expect.objectContaining({ resolvedDate: "2026-06-15", mealSlot: "cafe_da_manha", timezoneSource: "fallback" }) }));
  });

  it("aplica correção clara de quantidade sem criar nova refeição", async () => {
  ...
  it("mantem seleção destrutiva interativa para alvo ambíguo", async () => {
    listMealsMock.mockResolvedValue([recentMealWithChickenOptions()]);
    const first = await simulateWhatsappInbound(421, { text: "remove frango", receivedAt: new Date("2026-06-14T15:00:00.000Z"), messageId: "ctx-3-1" });
    expect(first).toEqual(expect.objectContaining({ action: "clarification_needed", eventType: "whatsapp.intent.delete_food_selection_requested", interactiveReply: expect.any(Object) }));
    expect(processMealDraftMock).not.toHaveBeenCalled();
  });

  it("mantem pendência destrutiva após seleção inicial ambígua", async () => {
    listMealsMock.mockResolvedValue([recentMealWithChickenOptions()]);
    const first = await simulateWhatsappInbound(422, { text: "remove frango", receivedAt: new Date("2026-06-14T15:00:00.000Z"), messageId: "ctx-4-1" });
    expect(first).toEqual(expect.objectContaining({ action: "clarification_needed", eventType: "whatsapp.intent.delete_food_selection_requested", data: expect.objectContaining({ destructiveActionBlocked: true }) }));
    expect(processMealDraftMock).not.toHaveBeenCalled();
  });

  it("roteia ajuste de registro pelo handler canônico sem criar nova refeição", async () => {
    listMealsMock.mockResolvedValue([recentMeal()]);
    const result = await simulateWhatsappInbound(42, { text: "troca arroz branco por arroz integral", receivedAt: new Date("2026-06-14T15:00:00.000Z") });
    expect(result).toEqual(expect.objectContaining({ handled: true, action: "meal_item_replaced", reply: expect.stringContaining("Arroz integral") }));
    expect(updateMealMock).toHaveBeenCalledOnce();
    expect(executeWhatsappLlmIntentMock).not.toHaveBeenCalled();
    expect(processMealDraftMock).not.toHaveBeenCalled();
  });
