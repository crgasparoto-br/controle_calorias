...
import type { AppendMessageInput, DomainLinkInput, WhatsAppConversationRepository } from "./repositories/whatsappConversationRepository";
...
vi.mock("./modules/quickEdit/service", () => ({ tryCreateQuickEditLinkForMeal: vi.fn(async () => null) }));

const { createMessageLifecycleService, withMessageLifecycleService } = await import("./modules/whatsapp/messageLifecycle");
...
type SharedState = {
  ...
  processingClaims: Map<number, { ownerToken: string; heartbeatAt: Date }>;
  nextMessageId: number;
};

function createSharedState(): SharedState {
  return { messages: [], domainLinks: [], processingClaims: new Map(), nextMessageId: 1 };
}
...
function createInMemoryConversationRepository(state: SharedState): WhatsAppConversationRepository {
  return {
    async createOrGetActiveConversation(userId, whatsappConnectionId, phoneNumber, now = new Date()) {
      return { id: userId, userId, whatsappConnectionId, phoneNumber, status: "active", startedAt: now, lastActivityAt: now, endedAt: null, version: 0, createdAt: now, updatedAt: now } as never;
    },
    async appendMessage(input) {
      ...
      const message: StoredMessage = {
        id: state.nextMessageId++, conversationId: input.conversationId, userId: input.userId,
        direction: input.direction, externalMessageId: input.externalMessageId ?? null, idempotencyKey,
        contentType: input.contentType, text: input.text ?? null, respondsToMessageId: input.respondsToMessageId ?? null,
        occurredAt: input.occurredAt, processedAt: input.processedAt ?? null, createdAt: now, updatedAt: now,
      };
    ...
    async linkResponse() {},
    async linkDomainRecord(messageId, link) { state.domainLinks.push({ messageId, link }); },
    async findRecentMessages(conversationId, limit = 20) { return state.messages.filter(m => m.conversationId === conversationId).slice(-limit) as never; },
    async findRecentMessagesByUser(userId, limit = 20) { return state.messages.filter(m => m.userId === userId).slice(-limit) as never; },
    async findMessagesBefore(conversationId, beforeOccurredAt, beforeId, limit = 20) {
      return state.messages.filter(m => m.conversationId === conversationId && (m.occurredAt < beforeOccurredAt || m.id < beforeId)).slice(-limit) as never;
    },
    ...
    async markProcessed(messageId, processedAt = new Date()) {
      ...
      if (message) { message.processedAt = processedAt; message.updatedAt = processedAt; }
    },
...
function createClaimRepository(state: SharedState): WhatsAppProcessingClaimRepository {
  return {
    ...
    async releaseUnprocessedMessage(messageId, releasedAt) {
      const message = state.messages.find(candidate => candidate.id === messageId);
      if (!message || message.processedAt) return false;
      state.processingClaims.delete(messageId);
      message.updatedAt = releasedAt;
      return true;
    },
    async claimUnprocessedMessage(messageId, ownerToken, staleBefore, claimedAt = new Date()) {
      const message = state.messages.find(candidate => candidate.id === messageId);
      if (!message) return { state: "unavailable" };
      if (message.processedAt) { state.processingClaims.delete(messageId); return { state: "processed" }; }
      const active = state.processingClaims.get(messageId);
      if (!active) {
        state.processingClaims.set(messageId, { ownerToken, heartbeatAt: claimedAt });
        return { state: "claimed", ownerToken, heartbeatAt: claimedAt };
      }
      if (active.heartbeatAt < staleBefore) {
        state.processingClaims.set(messageId, { ownerToken, heartbeatAt: claimedAt });
        return { state: "recovered", ownerToken, heartbeatAt: claimedAt };
      }
      return { state: "inflight", heartbeatAt: active.heartbeatAt };
    },
    async heartbeatOwnedUnprocessedMessage(messageId, ownerToken, heartbeatAt = new Date()) {
      const message = state.messages.find(candidate => candidate.id === messageId);
      const active = state.processingClaims.get(messageId);
      if (!message || message.processedAt || active?.ownerToken !== ownerToken) return false;
      state.processingClaims.set(messageId, { ownerToken, heartbeatAt });
      return true;
    },
    async releaseOwnedUnprocessedMessage(messageId, ownerToken) {
      const active = state.processingClaims.get(messageId);
      if (active?.ownerToken !== ownerToken) return false;
      state.processingClaims.delete(messageId);
      return true;
    },
...
function createRequest(phone: string, message: Record<string, unknown>) {
  return { body: { entry: [{ changes: [{ value: { metadata: { phone_number_id: "phone-number-test", display_phone_number: "5511000000000" }, messages: [{ from: phone, ...message }] } }] }] } };
}
...
describe("Baseline do contrato de respostas do WhatsApp (issue #780)", () => {
  ...
  function outboundMessages() { return sends.filter(send => send.kind !== "read_receipt"); }
  ...
  beforeEach(() => {
    ...
    global.fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/messages")) {
        ...
        if (payload.status !== "read" && failMessageSends) return { ok: false, status: 500, statusText: "failed", json: async () => ({}) } as Response;
  ...
  async function deliver(state: SharedState, phone: string, message: Record<string, unknown>) {
    ...
    await withMessageLifecycleService(createRuntime(state), () => handleWhatsAppPersistentContextWebhook(createRequest(phone, message) as never, response as never));
  ...
  it("imagem de refeição: fast path sem ack, sequência física ordenada e uma resposta gravada", async () => {
    ...
    await deliver(state, phone, { id: "wamid-baseline-image-1", timestamp: "1783742400", type: "image", image: { id: "image-baseline-1", mime_type: "image/jpeg", caption: "almoço" } });
  ...
  it("texto nutricional pelo fallback: fast path sem ack e gravação única", async () => {
    ...
    const state = createSharedState();
    await deliver(state, phone, { id: "wamid-baseline-text-1", timestamp: "1783742500", type: "text", text: { body: "100g arroz branco" } });
    expect(processMealInputMock).toHaveBeenCalledTimes(1);
    expect(state.domainLinks.filter(entry => entry.link.mealId)).toHaveLength(1);
  ...
  it("água por texto: intent responde sem ack, registra hidratação uma vez e reentrega não repete efeitos", async () => {
    ...
    const state = createSharedState();
    const message = { id: "wamid-baseline-water-1", timestamp: "1783742600", type: "text", text: { body: "bebi 300 ml de água" } };
  ...
  it("solicitação informativa (peso sem valor): responde clarificação sem criar registro de domínio", async () => {
    ...
    const state = createSharedState();
    await deliver(state, phone, { id: "wamid-baseline-clarify-1", timestamp: "1783742700", type: "text", text: { body: "quero registrar meu peso" } });
  ...
  it("falha de envio: domínio executa uma vez, nada é gravado como resposta e a reentrega não reprocessa como refeição", async () => {
    ...
    const state = createSharedState();
    const message = { id: "wamid-baseline-water-failure", timestamp: "1783742800", type: "text", text: { body: "bebi 250 ml de água" } };
