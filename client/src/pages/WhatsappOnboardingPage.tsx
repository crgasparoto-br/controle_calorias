...
export default function WhatsappOnboardingPage() {
  ...
  const completeOnboarding =
    trpc.auth.whatsappOnboarding.complete.useMutation({
      onSuccess: result => {
        ...
        toast.info(
          "Cadastro concluído. Consulte Plano e acesso para acompanhar a ativação."
        );
        setLocation("/billing");
      },
      onError: error =>
  ...
  const validationMessage = useMemo(() => {
    ...
    if (!parsed.heightCm || parsed.heightCm < 100 || parsed.heightCm > 250)
      return "Informe uma altura entre 1,00 m e 2,50 m.";
    if (
      !parsed.currentWeightKg ||
      parsed.currentWeightKg < 25 ||
      parsed.currentWeightKg > 350
    )
      return "Informe seu peso atual.";
    if (
      !form.acceptedTerms ||
      !form.acceptedPrivacyPolicy ||
      !form.acceptedHealthDataProcessing ||
      !form.acceptedOperationalWhatsapp
    )
      return "Aceite os consentimentos obrigatórios para concluir.";
    return null;
  }, [form, parsed]);
  ...
  if (leadQuery.isLoading) {
    return (
      <StatusScreen
        ...
        description="Estamos conferindo se o convite recebido pelo WhatsApp ainda está ativo."
      />
    );
  }
      ...
      >
        <section className="grid gap-4 rounded-2xl border bg-muted/20 p-5 shadow-sm md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <p className="flex items-center gap-2 text-sm font-medium text-primary">
              <MessageCircle className="h-4 w-4" /> Cadastro iniciado pelo
              WhatsApp
            </p>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Complete os dados obrigatórios. Depois do cadastro, o backend
              informará se o acesso já está liberado ou se ainda aguarda uma
          ...
          <div className="rounded-xl border bg-background px-4 py-3 text-sm">
            ...
            <p className="mt-1 font-semibold">
              {leadQuery.data.phoneNumberMasked}
            </p>
          </div>
        </section>
        ...
        <Card className="border shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Esses dados criam sua conta web e calibram as metas iniciais.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="border shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Use respostas simples. Você pode ajustar tudo depois nas
              configurações.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <SelectField
              ...
              onChange={value =>
                updateField("objective", value as FormState["objective"])
              }
            />
            <SelectField
              ...
              onChange={value =>
                updateField(
                  "activityLevel",
                  value as FormState["activityLevel"]
                )
              }
            />
            <SelectField
              label="Experiência"
              value={form.trackingExperience}
              options={EXPERIENCE_OPTIONS}
              onChange={value =>
                updateField(
                  "trackingExperience",
                  value as FormState["trackingExperience"]
                )
              }
            />
            <SelectField
              label="Rotina alimentar"
              value={form.eatingRoutine}
              options={ROUTINE_OPTIONS}
              onChange={value =>
                updateField(
                  "eatingRoutine",
                  value as FormState["eatingRoutine"]
                )
              }
            />
            <SelectField
              ...
              onChange={value =>
                updateField(
                  "mainDifficulty",
                  value as FormState["mainDifficulty"]
                )
              }
            />
            <div className="grid gap-4 md:col-span-2 xl:col-span-3 xl:grid-cols-2">
              <TextAreaField
                ...
                placeholder="Ex.: comida caseira, vegetariano"
              />
              <TextAreaField
        ...
        <Card className="border shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Os aceites obrigatórios registram consentimento para uso do
              serviço e mensagens operacionais pelo WhatsApp.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <ConsentField
              ...
              onChange={value => updateField("acceptedPrivacyPolicy", value)}
              label="Aceito a política de privacidade."
            />
            <ConsentField
              checked={form.acceptedHealthDataProcessing}
              onChange={value =>
                updateField("acceptedHealthDataProcessing", value)
              }
              label="Autorizo o tratamento dos dados necessários para calcular metas, registrar refeições e acompanhar evolução."
            />
            <ConsentField
              checked={form.acceptedOperationalWhatsapp}
              onChange={value =>
                updateField("acceptedOperationalWhatsapp", value)
              }
              label="Autorizo mensagens operacionais pelo WhatsApp, como respostas aos meus registros e avisos do serviço."
            />
            <ConsentField
              checked={form.acceptedMarketingWhatsapp}
              onChange={value =>
                updateField("acceptedMarketingWhatsapp", value)
              }
              label="Aceito receber comunicações de marketing pelo WhatsApp."
              optional
            />
          </CardContent>
        </Card>
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="max-w-2xl text-sm text-muted-foreground">
            Ao concluir, a conta e o vínculo do WhatsApp serão criados. A
            liberação dos recursos será confirmada pelo backend e poderá aparecer
            como pendente em Plano e acesso.
          </p>
          <Button
            type="submit"
