...
function accessDateLabel(access: PatientAccess, timeZone: string) {
...
function dateTimeLabel(value: string | number | Date | null | undefined, timeZone: string) {
  return value ? formatDateTimeInTimeZone(value, timeZone) : "Sem data";
}
...
export default function ProfessionalReportsPage() {
  ...
  const dashboard = trpc.nutrition.professionals.patientDashboard.useQuery({ patientId: selectedPatientId ?? 0, weekOffset }, { enabled: hasActiveProfile && patientTimeZoneReady });
  const periodBundle = trpc.nutrition.professionals.patientPeriodBundle.useQuery({ patientId: selectedPatientId ?? 0, startDate: activeRange.start, endDate: activeRange.end }, { enabled: hasActiveProfile && patientTimeZoneReady && periodScope !== "week" });
  ...
  React.useEffect(() => {
    if (!patientTimeZoneReady) return;
...
function AccessRow({ access, selected, onSelect, onRevoke, revoking, timeZone }: { access: PatientAccess; selected: boolean; onSelect: () => void; onRevoke: () => void; revoking: boolean; timeZone: string }) { return <div className={`rounded-2xl border bg-background p-4 ${selected ? "ring-2 ring-primary/30" : ""}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium">{personLabel(access)}</p><p className="text-xs text-muted-foreground">{access.patient?.email || `ID interno #${access.patientUserId}`}</p><p className="text-xs text-muted-foreground">{accessDateLabel(access, timeZone)}</p></div><div className="flex flex-wrap gap-2"><Button variant={selected ? "default" : "outline"} className="rounded-full" onClick={onSelect}>Analisar</Button><Button variant="outline" className="rounded-full" onClick={onRevoke} disabled={revoking}><X className="mr-2 h-4 w-4" /> Revogar vínculo</Button></div></div></div>; }
...
function GoalSuggestionRow({ suggestion, timeZone }: { suggestion: GoalSuggestion; timeZone: string }) { const goal = normalizeGoalTarget(suggestion.goal?.defaultGoal); return <div className="rounded-xl border bg-muted/20 p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium">{suggestionStatusLabel(suggestion.status)}</span><span className="text-xs text-muted-foreground">{dateTimeLabel(suggestion.createdAt, timeZone)}</span></div><div className="mt-2 grid gap-2 text-muted-foreground md:grid-cols-4"><span>{formatCalories(goal.calories)}</span><span>{formatGrams(goal.proteinGrams)} proteína</span><span>{formatGrams(goal.carbsGrams)} carboidratos</span><span>{formatGrams(goal.fatGrams)} gorduras</span></div><p className="mt-2 text-muted-foreground">{suggestion.rationale || "Sugestão sem justificativa registrada."}</p></div>; }
function MealSuggestionRow({ suggestion, timeZone }: { suggestion: MealSuggestion; timeZone: string }) { return <div className="rounded-xl border bg-muted/20 p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium">{suggestion.mealLabel || "Refeição"} · {suggestion.title || "Sugestão"}</span><span className="text-xs text-muted-foreground">{suggestionStatusLabel(suggestion.status)} · {dateTimeLabel(suggestion.createdAt, timeZone)}</span></div><p className="mt-2 text-muted-foreground">{suggestion.description || "Sem descrição."}</p><p className="mt-2 text-muted-foreground">Justificativa: {suggestion.rationale || "não informada"}</p>{suggestion.notes ? <p className="mt-2 text-xs text-muted-foreground">Obs.: {suggestion.notes}</p> : null}</div>; }
...
function CommentRow({ comment, timeZone }: { comment: ProfessionalComment; timeZone: string }) { return <div className="rounded-xl border bg-muted/20 p-3 text-sm"><p>{comment.comment || "Comentário sem texto."}</p>{comment.createdAt ? <p className="mt-2 text-xs text-muted-foreground">{formatDateTimeInTimeZone(comment.createdAt, timeZone)}</p> : null}</div>; }
