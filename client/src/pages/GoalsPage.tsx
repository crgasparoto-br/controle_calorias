...
function resolvePreviewException(
  exceptions: GoalExceptionForm[],
  weekday: number,
  dateKey: string
) {
  return exceptions
...
function formatMacroPercentSummary(
  goal: Pick<GoalTargetForm, MacroPercentField>
) {
...
function getDurationLabel(durationType: DurationType) {
  return (
    DURATION_OPTIONS.find(option => option.value === durationType)?.label ??
    "Duração definida"
  );
}
...
function applyPercentagesToGoal(
  goal: GoalTargetForm,
  percentages: Pick<GoalTargetForm, MacroPercentField>
): GoalTargetForm {
...
function createGoalTargetForm(
  goal: GoalTargetBase,
  inputMode: MacroInputMode = "grams"
): GoalTargetForm {
  ...
  return inputMode === "percent"
    ? applyPercentagesToGoal(form, percentages)
    : form;
}
...
function exceptionVersionKey(
  exception: Pick<GoalExceptionForm, "weekday" | "startDate">,
  fallbackStartDate: string
) {
  return `${exception.weekday}:${exception.startDate || fallbackStartDate}`;
}

function normalizeExceptionsForEditing(
  exceptions: GoalExceptionQuery[],
  fallbackStartDate: string
) {
  const editableExceptions = exceptions.some(
    exception => typeof exception.isActive === "boolean"
  )
  ...
  return sorted.filter(exception => {
    const startDate =
      exception.startDate ??
      dateKeyFromDateLike(exception.effectiveFrom) ??
      fallbackStartDate;
...
function Switch({
  ...
  return (
    ...
    >
      <span
        className={`inline-block h-4 w-4 transform rounded-full bg-background shadow transition-transform ${checked ? "translate-x-6" : "translate-x-1"}`}
      />
    </button>
  );
}

function ExerciseCaloriesToggle({
  checked,
  onChange,
}: {
  checked: boolean;
  ...
  return (
    <div className="rounded-2xl border bg-background p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Label htmlFor="include-exercise-calories">
            Adicionar calorias dos exercícios à meta diária
          </Label>
          <p className="mt-1 text-sm text-muted-foreground">
            Ative esta opção quando sua meta não considerar os exercícios. Se a
            meta informada já incluir sua rotina de atividades físicas, mantenha
            esta opção desativada.
          </p>
        </div>
        <Switch
          id="include-exercise-calories"
          checked={checked}
          onChange={onChange}
        />
      </div>
...
export default function GoalsPage() {
  ...
  const [includeExerciseCalories, setIncludeExerciseCalories] = useState(
    () => goalQuery.data?.today.includeExerciseCalories ?? true
  );
  const [defaultGoal, setDefaultGoal] = useState<GoalTargetForm>(() =>
    goalQuery.data
      ...
      : createGoalTargetForm(DEFAULT_GOAL_BASE, "percent")
  );
  const [exceptions, setExceptions] = useState<GoalExceptionForm[]>(() => {
    ...
    return goalQuery.data
      ? normalizeExceptionsForEditing(
          goalQuery.data.exceptions,
          fallbackStartDate
        ).map(exception => ({
          id: exception.id,
  ...
  const exceptionVersions = (goalQuery.data?.exceptionVersions ??
    []) as GoalExceptionVersionQuery[];
  const previewWeekStartDate = useMemo(
    () => startOfPreviewWeekDateKey(startDate),
    [startDate]
  );
  const previewWeekEndDate = useMemo(
    () => addDaysToDateKey(previewWeekStartDate, 6),
    [previewWeekStartDate]
  );
  const previewDays = useMemo(
    () =>
      WEEKDAY_META.map((day, index) => {
        const date = addDaysToDateKey(previewWeekStartDate, index);
        const exception = resolvePreviewException(
          exceptions,
          day.weekday,
          date
        );
  ...
  const hasInvalidPercentages =
    !isPercentModeValid(defaultGoal) ||
    exceptions.some(exception => !isPercentModeValid(exception));
    ...
    () =>
      assessNutritionGoalTargets([
        ...
        ...exceptions.map(exception => ({
          label:
            WEEKDAY_META.find(day => day.weekday === exception.weekday)
              ?.label ?? "Exceção",
  ...
  function updateGoalTargetField(
    current: GoalTargetForm,
    field: keyof GoalPayload,
    value: number
  ) {
  ...
  function updateGoalTargetPercent(
    current: GoalTargetForm,
    field: MacroPercentField,
    value: number
  ) {
    return applyPercentagesToGoal(
      { ...current, inputMode: "percent" },
      {
        proteinPercent:
          field === "proteinPercent" ? value : current.proteinPercent,
  ...
  function updateExceptionPercent(
    index: number,
    field: MacroPercentField,
    value: number
  ) {
  ...
  function addException() {
    ...
    setExceptions(current => [
      ...current,
      buildException(nextDay.weekday, startDate),
    ]);
  }
  ...
  function handleSave() {
    if (hasInvalidPercentages) {
      toast.error(
        "Quando o modo percentual estiver ativo, a soma de proteínas, carboidratos e gorduras precisa fechar em 100%."
      );
      return;
    }
    ...
    if (hasExceptionVersionConflict) {
      toast.error(
        "Há duas exceções para o mesmo dia com a mesma data de início."
      );
      return;
    }
      ...
      <div className="space-y-6">
          ...
          stats={
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              ...
              <IntroStat
                ...
                supporting={
                  exceptions.length
                    ? "regras com dia e início próprios"
                    : "meta padrão em todos os dias"
                }
              />
              <IntroStat
        ...
        <div className="space-y-6">
          <Card className="border-0 shadow-sm">
            <CardHeader className="gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1.5">
                ...
                <CardDescription>
                  Use esta meta como referência para os dias sem exceção.
                  Preencha calorias e macronutrientes, informe a data de início
                  e revise os avisos antes de salvar.
                </CardDescription>
              </div>
              <Button
            ...
            <CardContent className="space-y-4">
              ...
              <ExerciseCaloriesToggle
                checked={includeExerciseCalories}
                onChange={setIncludeExerciseCalories}
              />
              <ModeSelector
                mode={defaultGoal.inputMode}
                onChange={updateDefaultInputMode}
              />
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <FormattedField
                  label="Calorias"
                  value={defaultGoal.calories}
                  onChange={value => updateDefaultGoal("calories", value)}
                  suffix="kcal"
                />
                <MacroField
                  label="Proteínas"
                  ...
                  onGramChange={value =>
                    updateDefaultGoal("proteinGrams", value)
                  }
                  onPercentChange={value =>
                    updateDefaultGoalPercent("proteinPercent", value)
                  }
                />
                <MacroField
                  label="Carboidratos"
                  ...
                  onPercentChange={value =>
                    updateDefaultGoalPercent("carbsPercent", value)
                  }
                />
                <MacroField
                  label="Gorduras"
                  ...
                  onPercentChange={value =>
                    updateDefaultGoalPercent("fatPercent", value)
                  }
                />
              </div>
              <PercentValidationNote
                mode={defaultGoal.inputMode}
                percentSum={defaultPercentSum}
              />
              <NutritionSafetyNotice issues={safetyAssessment.issues} />
              <p className="text-xs text-muted-foreground">
                Metas muito extremas são bloqueadas para proteger sua saúde.
                Ajuste esses valores antes de salvar.
              </p>
            </CardContent>
          </Card>
          ...
          >
            <CardHeader>
              ...
              <CardDescription>
                Simulação de {formatDateKey(previewWeekStartDate)} a{" "}
                {formatDateKey(previewWeekEndDate)}. Para cada dia, vale a
                exceção programada mais recente que já iniciou naquela data.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid auto-cols-[minmax(10rem,1fr)] grid-flow-col gap-3 overflow-x-auto pb-2 xl:grid-flow-row xl:grid-cols-3 xl:overflow-visible xl:pb-0">
                {previewDays.map(day => (
                  <div
                    ...
                    className="min-w-0 rounded-2xl border border-l-4 border-l-emerald-500 bg-background p-3"
                  >
                    <div className="space-y-2">
                      ...
                      <p className="min-h-10 text-sm leading-5 text-foreground">
                        {day.source === "exception"
                          ? `Exceção desde ${formatDateKey(day.startDate)}.`
                          : "Usa a meta padrão."}
                      </p>
                    </div>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1.5">
                ...
                <CardDescription>
                  Configure regras com dia da semana, data de início e duração
                  para treinos, descanso, fins de semana ou compromissos
                  especiais.
                </CardDescription>
              </div>
              <Button
            ...
            <CardContent className="space-y-4">
              {hasExceptionVersionConflict ? (
                <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                  Há duas exceções para o mesmo dia com a mesma data de início.
                  Altere uma das datas antes de salvar.
                </div>
              ) : null}
              ...
              {exceptions.length ? (
                exceptions.map((exception, index) => (
                  <div
                    key={`${exception.weekday}-${exception.startDate}-${index}`}
                  ...
                  >
                    <div className="grid gap-3 lg:grid-cols-[1fr,1fr,1fr,auto] lg:items-end">
                      <SelectField
                        ...
                        options={WEEKDAY_META.map(day => ({
                          value: String(day.weekday),
                          label: day.label,
                        }))}
                        onChange={value =>
                          updateException(index, { weekday: Number(value) })
                        }
                      />
                      <DateField
                        ...
                        onChange={value =>
                          updateException(index, { startDate: value })
                        }
                      />
                      <SelectField
                        label="Duração"
                    ...
                    <div className="mt-3 rounded-2xl bg-background/70 p-3 text-sm text-muted-foreground">
                      <p>
                        {getWeekdayLabel(exception.weekday)} desde{" "}
                        {formatDateKey(exception.startDate)} ·{" "}
                        {getDurationLabel(exception.durationType)}
                      </p>
                      <p className="mt-1 text-foreground">
                        {formatCalories(exception.calories)} ·{" "}
                        {formatMacroSummary(exception)}
                      </p>
                    </div>
                    <div className="mt-4 space-y-4">
                      <ModeSelector
                        mode={exception.inputMode}
                        onChange={mode => updateExceptionInputMode(index, mode)}
                      />
                      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        <FormattedField
                          label="Calorias"
                          value={exception.calories}
                          onChange={value =>
                            updateExceptionField(index, "calories", value)
                          }
                          suffix="kcal"
                        />
                        <MacroField
                          ...
                          onGramChange={value =>
                            updateExceptionField(index, "proteinGrams", value)
                          }
                          onPercentChange={value =>
                            updateExceptionPercent(
                              index,
                              "proteinPercent",
                              value
                            )
                        ...
                        <MacroField
                          ...
                          onGramChange={value =>
                            updateExceptionField(index, "carbsGrams", value)
                          }
                          onPercentChange={value =>
                            updateExceptionPercent(index, "carbsPercent", value)
                          }
                        />
                        <MacroField
                          label="Gorduras"
                          ...
                          onGramChange={value =>
                            updateExceptionField(index, "fatGrams", value)
                          }
                          onPercentChange={value =>
                            updateExceptionPercent(index, "fatPercent", value)
                          }
                        />
                      </div>
                      <PercentValidationNote
                        mode={exception.inputMode}
                        percentSum={getPercentSum(exception)}
                      />
                    </div>
                  </div>
                ))
                ...
                <div className="rounded-3xl border border-dashed bg-muted/20 p-6 text-sm text-muted-foreground">
                  ...
                  <p className="mt-1">
                    A meta padrão será usada todos os dias. Use exceções para
                    dias de treino, descanso, fim de semana ou compromissos
                    especiais.
                  </p>
                  <Button
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Consulte versões anteriores e programadas sem misturar histórico
              com a edição principal.
            </p>
          </div>
          <div className="grid gap-6 xl:grid-cols-2">
            <GoalVersionHistory
              versions={goalVersions}
              selectedStartDate={startDate}
            />
            <ExceptionVersionHistory
              versions={exceptionVersions}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.8fr),minmax(0,1.2fr)] lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            A meta salva passa a valer a partir desta data. Dias anteriores
            continuam usando a versão que já estava vigente.
          </p>
        </div>
        <DateField
          id="goal-start-date"
...
function GoalVersionHistory({
  versions,
  selectedStartDate,
}: {
  versions: GoalVersionQuery[];
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Confira as datas de início já cadastradas para evitar versões
          duplicadas e entender o histórico aplicado aos dias anteriores.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {versions.length ? (
          ...
          <div className="rounded-2xl border border-dashed bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
            Nenhuma versão salva foi encontrada ainda. Ao salvar a meta, a
            primeira versão aparecerá aqui com a data de início escolhida.
          </div>
        )}
      </CardContent>
    </Card>
...
function ExceptionVersionHistory({
  versions,
  selectedExceptions,
}: {
  versions: GoalExceptionVersionQuery[];
  selectedExceptions: GoalExceptionForm[];
}) {
  return (
    <Card className="border-0 shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Acompanhe quando cada exceção começa e termina. Exceções vigentes têm
          prioridade sobre a meta geral no dia correspondente.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {versions.length ? (
          versions.map(version => {
            const isSelected = selectedExceptions.some(
              exception =>
                exception.weekday === version.weekday &&
                exception.startDate === version.startDate
            );
            return (
              <VersionHistoryItem
                key={version.id}
                ...
                detail={
                  DURATION_OPTIONS.find(
                    option => option.value === version.durationType
                  )?.label
                }
              />
            );
          ...
          <div className="rounded-2xl border border-dashed bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
            Nenhuma exceção versionada foi encontrada ainda. Ao salvar uma
            exceção com data de início, ela aparecerá aqui.
          </div>
        )}
      </CardContent>
    </Card>
...
function VersionHistoryItem({
  ...
  return (
    <div
      className={`rounded-2xl border bg-background p-4 shadow-sm ${isSelected ? "border-primary/50" : ""}`}
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-medium tracking-tight">{title}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {formatVersionEndDate(endDate)}
          </p>
          {detail ? (
...
function NutritionSafetyNotice({
  issues,
}: {
  ...
  return (
    <div
      className={`rounded-2xl border p-4 text-sm ${blockers.length ? "border-destructive/30 bg-destructive/5 text-destructive" : "border-amber-200 bg-amber-50 text-amber-800"}`}
    >
      <div className="flex items-start gap-3">
        <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
        <div className="space-y-2">
          <p className="font-medium tracking-tight">
            Revisão de segurança nutricional
          </p>
          <p
            className={blockers.length ? "text-destructive" : "text-amber-800"}
          >
            {blockers.length
          ...
          <ul className="space-y-1">
            {[...blockers, ...warnings].slice(0, 4).map(issue => (
              <li key={`${issue.code}-${issue.targetLabel}-${issue.message}`}>
                {issue.message}
              </li>
            ))}
          </ul>
...
function ModeSelector({
  ...
  mode: MacroInputMode;
  onChange: (mode: MacroInputMode) => void;
}) {
  return (
    <div className="rounded-2xl border bg-background p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
...
function MacroField({
  ...
  return (
    <div className="space-y-2 rounded-2xl border bg-background p-4">
      ...
      <div className="flex items-center gap-3">
        {mode === "grams" ? (
          <Input
            ...
            onChange={event =>
              onGramChange(parseIntegerInputPtBr(event.target.value))
            }
          />
        ) : (
          <Input
      ...
      {mode === "percent" ? (
        <p className="text-xs text-muted-foreground">
          Calculado automaticamente pela meta calórica: {formatGrams(grams)}
        </p>
      ) : (
        <p className="text-xs text-muted-foreground">
          Informe a quantidade planejada em gramas.
        </p>
      )}
    </div>
  );
    ...
    <div className="space-y-2 rounded-2xl border bg-background p-4">
      ...
      <div className="flex items-center gap-3">
        <Input
          ...
          onChange={event =>
            onChange(parseIntegerInputPtBr(event.target.value))
          }
        />
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p>
            A soma atual é de{" "}
            <strong>{formatPercentPtBr(percentSum, 1)}%</strong>. Para salvar
            por percentual, proteínas, carboidratos e gorduras precisam somar
            exatamente <strong>100%</strong>.
          </p>
        </div>
      </div>
    </div>
