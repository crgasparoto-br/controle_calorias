...
describe("ReportsGoalsPage weight trend", () => {
  beforeEach(() => {
    reportWeightEntries.current = [];
  });

  it("mantém a seção visível sem registros de peso", async () => {
    const { default: ReportsGoalsPage } = await import("./ReportsGoalsPage");
    const html = renderToString(React.createElement(ReportsGoalsPage));
