...
function validValue<T extends string>(value: string | null, allowed: readonly T[], fallback: T) {
...
export default function ProfessionalPatients() {
  ...
  const [filters, setFilters] = useState<Filters>(() => filtersFromLocation(location));
    ...
    <ProfessionalPage>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_auto] lg:items-end">
            <label className="grid gap-1 text-sm">
              ...
              <Input value={patientContact} onChange={event => setPatientContact(event.target.value)} placeholder="paciente@exemplo.com ou celular" />
            </label>
            <label className="grid gap-1 text-sm">
              <span className="font-medium">Motivo da solicitação</span>
              <Input value={reason} onChange={event => setReason(event.target.value)} placeholder="Ex.: iniciar acompanhamento nutricional" />
            </label>
            <Button
              disabled={patientContact.trim().length < 3 || reason.trim().length < 3 || requestAccess.isPending}
              onClick={() => {
                setRequestSuccess(null);
                requestAccess.reset();
                requestAccess.mutate({ patientContact: patientContact.trim(), reason: reason.trim() });
              }}
            >
              {requestAccess.isPending ? "Enviando..." : "Enviar solicitação"}
            </Button>
            {requestSuccess ? <p role="status" className="text-sm text-emerald-700 lg:col-span-3 dark:text-emerald-300">{requestSuccess}</p> : null}
            {requestAccess.isError ? <p role="alert" className="text-sm text-destructive lg:col-span-3">{requestAccessErrorMessage(requestAccess.error)}</p> : null}
          </CardContent>
        </Card>
      ) : null}
      ...
      <section aria-label="Filtros da carteira" className="grid min-w-0 gap-3 rounded-2xl border bg-card p-4 md:grid-cols-2 xl:grid-cols-[minmax(260px,1.4fr)_repeat(4,minmax(170px,1fr))]">
        <label className="relative min-w-0">
          ...
          <Input className="pl-9" value={searchInput} onChange={event => setSearchInput(event.target.value)} placeholder="Nome, e-mail ou identificador" />
        </label>
        <select aria-label="Filtrar autorização" className="h-10 min-w-0 rounded-md border bg-background px-3 text-sm" value={filters.authorizationStatus} onChange={event => setFilters(current => ({ ...current, authorizationStatus: event.target.value as Filters["authorizationStatus"], page: 1 }))}>
        ...
        <select aria-label="Filtrar acompanhamento" className="h-10 min-w-0 rounded-md border bg-background px-3 text-sm" value={filters.trackingStatus} onChange={event => setFilters(current => ({ ...current, trackingStatus: event.target.value as Filters["trackingStatus"], page: 1 }))}>
        ...
        <select aria-label="Filtrar atividade" className="h-10 min-w-0 rounded-md border bg-background px-3 text-sm" value={filters.activity} onChange={event => setFilters(current => ({ ...current, activity: event.target.value as Filters["activity"], page: 1 }))}>
        ...
        <select aria-label="Filtrar próxima revisão" className="h-10 min-w-0 rounded-md border bg-background px-3 text-sm" value={filters.nextReview} onChange={event => setFilters(current => ({ ...current, nextReview: event.target.value as Filters["nextReview"], page: 1 }))}>
      ...
      {openError ? <p role="alert" className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-sm text-destructive">{openError}</p> : null}
      {portfolio.isLoading ? (
        <ProfessionalLoadingState label="Carregando carteira profissional..." />
      ) : portfolio.isError ? (
        <ProfessionalAsyncState title="Não foi possível carregar a carteira" description="Nenhum dado de paciente foi exibido." onRetry={() => void portfolio.refetch()} />
      ) : portfolio.data?.items.length === 0 ? (
        <ProfessionalAsyncState icon="empty" title="Nenhum paciente encontrado" description="Ajuste os filtros ou solicite acesso a um novo paciente." />
      ) : (
        <div className="grid gap-3">
          {portfolio.data?.items.map((item: any) => {
            const accessible = item.authorizationStatus === "approved";
            const displayName = item.patientName ?? (accessible ? item.patientEmail : null) ?? `Paciente ${item.patientUserId}`;
            return (
              <article key={item.authorizationId} className="grid min-w-0 gap-4 rounded-2xl border bg-card p-4 shadow-sm lg:grid-cols-[minmax(220px,1.4fr)_minmax(150px,.8fr)_minmax(150px,.8fr)_minmax(200px,1fr)_auto] lg:items-center">
                <div className="min-w-0">
                  <h2 className="break-words font-semibold">{displayName}</h2>
                  <p className="mt-1 break-words text-sm text-muted-foreground">
                    {accessible ? item.patientEmail ?? "Identificação não informada" : "Dados pessoais e clínicos disponíveis após autorização"}
                  </p>
                </div>
                <div className="min-w-0">
                  <p className="mb-1 text-xs text-muted-foreground">Autorização</p>
                  <ProfessionalStatusBadge kind="authorization" value={item.authorizationStatus} />
                </div>
                <div className="min-w-0">
                  <p className="mb-1 text-xs text-muted-foreground">Acompanhamento</p>
                  {accessible ? <ProfessionalStatusBadge kind="tracking" value={item.trackingStatus ?? "not_started"} /> : <p className="text-sm font-medium text-muted-foreground">Disponível após autorização</p>}
                </div>
                <dl className="min-w-0 space-y-2 text-sm">
                  <div><dt className="text-xs text-muted-foreground">Última atividade</dt><dd className="break-words font-medium">{accessible ? formatDate(item.lastFoodActivityAt, "Não informado") : "Disponível após autorização"}</dd></div>
                  <div><dt className="text-xs text-muted-foreground">Próxima revisão</dt><dd className="break-words font-medium">{accessible ? formatDate(item.nextReviewAt, "Sem revisão agendada") : "Disponível após autorização"}</dd></div>
                </dl>
                <Button variant={accessible ? "default" : "outline"} disabled={!accessible || openingPatientId !== null} onClick={() => void openPatient(item)}>
                  {openingPatientId === item.patientUserId ? "Validando..." : accessible ? "Abrir paciente" : unavailableActionLabel(item.authorizationStatus)}
                </Button>
              </article>
            );
          })}
      ...
      {portfolio.data && portfolio.data.pagination.totalPages > 1 ? (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">Página {portfolio.data.pagination.page} de {portfolio.data.pagination.totalPages}</p>
          <div className="flex gap-2">
            <Button variant="outline" disabled={filters.page <= 1} onClick={() => setFilters(current => ({ ...current, page: current.page - 1 }))}>
              <ChevronLeft className="h-4 w-4" />Anterior
            </Button>
            <Button variant="outline" disabled={filters.page >= portfolio.data.pagination.totalPages} onClick={() => setFilters(current => ({ ...current, page: current.page + 1 }))}>
