        ...
        {distribution.length ? (
          <div className="grid gap-3 md:grid-cols-3">
            {distribution.map(item => (
              <div key={item.key} className="rounded-2xl border bg-background p-4 shadow-sm">
                <div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-medium">{item.label}</p><Badge variant="outline">{formatPercent(item.percent)}</Badge></div>
...
function DailyDetailsSections({ groups, userTimeZone }: { groups: DateGroupedRegisteredMealsViewModel[]; userTimeZone: string }) {
  if (!groups.length) return <ReportEmptyState text="Nenhuma refeição confirmada foi encontrada para detalhamento neste intervalo." />;
  return (
    <Card className="border-0 shadow-sm">
      <CardHeader>
...
export default function ReportsPage() {
  ...
  const activeLoading = isWeek ? reportBundle.isLoading : periodBundle.isLoading;
  const activeError = isWeek ? reportBundle.isError : periodBundle.isError;
  const weeklyDays = React.useMemo(() => ((reportBundle.data?.weekly ?? []) as any[]).map(day => normalizeDay(day)), [reportBundle.data?.weekly]);
  const periodDays = React.useMemo(() => ((periodBundle.data?.daily ?? []) as any[]).map(day => normalizeDay(day, periodBundle.data?.goal)), [periodBundle.data?.daily, periodBundle.data?.goal]);
  const metricDays = isWeek ? weeklyDays : periodDays;
  ...
  const mealGroupsAsc = React.useMemo(
    () => buildMealsByDate((reportBundle.data?.mealsByDate ?? []) as MealDateGroup[], periodMeals, periodScope, userTimeZone),
  ...
  const fallbackTrendData = !trendData.length && periodScope !== "week" ? buildTrendFromMealGroups(mealGroupsAsc, periodBundle.data?.goal) : trendData;
  const visibleTrendData = trendData.length ? trendData : fallbackTrendData;
  const adherence = calculateCalorieAdherence(visibleTrendData, dayCount);
  const diagnosis = buildDiagnosis(periodScope, adherence.adherencePercent, adherence.daysWithinRange, dayCount);

  const totals = metricDays.length
    ? metricDays.reduce<Totals>((acc, day) => ({ calories: acc.calories + day.calories, protein: acc.protein + day.protein, carbs: acc.carbs + day.carbs, fat: acc.fat + day.fat }), { ...EMPTY_TOTALS })
  ...
  const plannedMacros: MacroTotals = {
    protein: metricDays.reduce((total, day) => total + day.goalProtein, 0),
  ...
  const foodQuality = (isWeek ? (weeklyQualityRaw as any)?.foodQuality : (periodBundle.data as any)?.quality?.foodQuality) as FoodQualitySummary | undefined;
  const waterConsumedMl = isWeek ? metricDays.reduce((total, day) => total + day.waterConsumedMl, 0) : numberValue((periodBundle.data as any)?.habitAnalytics?.water?.totalConsumedMl);
  const waterGoalMl = isWeek ? metricDays.reduce((total, day) => total + day.waterGoalMl, 0) : numberValue((periodBundle.data as any)?.habitAnalytics?.water?.totalGoalMl);
  const waterHitDays = isWeek ? metricDays.filter(day => day.waterGoalMl > 0 && day.waterConsumedMl >= day.waterGoalMl).length : numberValue((periodBundle.data as any)?.habitAnalytics?.water?.goalHitDays);
