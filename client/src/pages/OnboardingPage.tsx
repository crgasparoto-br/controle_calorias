...
type FormState = {
  ...
  timezone: typeof USER_TIME_ZONE_OPTIONS[number]["value"];
};
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      timezone: (profile.timezone as FormState["timezone"] | undefined) ?? DEFAULT_APP_TIME_ZONE,
    });
  ...
  const validationMessage = useMemo(() => {
    ...
    return null;
  }, [calculatedAgeYears, form.heightCm, parsed, phoneCountryCode, phoneNationalNumber, shouldAttachWhatsappPhone]);
  ...
  const completeOnboarding = trpc.nutrition.onboarding.complete.useMutation({
    onSuccess: async () => {
      ...
      } else if (sendWhatsappGreeting && hasWhatsappConnection) {
        try {
          await sendGreetingToast();
        } catch (error) {
              ...
              <Card className="border-0 shadow-sm">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="rounded-xl border bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
                    O WhatsApp é o canal principal para registrar refeições, água e exercícios. A saudação é uma mensagem única de boas-vindas e não ativa mensagens de marketing.
                    {contactPhoneNumber ? ` Será enviada para ${contactPhoneNumber}.` : ""}
                  </div>
                </CardContent>
              </Card>
            ) : null}
...
function SelectField<T extends readonly { value: string; label: string }[]>({ label, value, options, onChange, helper }: {
  label: string;
  value: string;
  options: T;
  onChange: (value: string) => void;
