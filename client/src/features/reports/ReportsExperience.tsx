...
export function ReportsExperience({ context = "self", subjectUserId }: ReportsExperienceProps) {
  ...
  const selfWeeklySummary = normalizeQueryResult(trpc.nutrition.reports.weekly.useQuery({ weekOffset }, { enabled: !isProfessional && periodScope === "week" }));
  const selfWeeklyDetails = normalizeQueryResult(trpc.nutrition.reports.bundle.useQuery({ weekOffset }, { enabled: !isProfessional && periodScope === "week" && showDetails }));
  ...
  const activeBundle = isProfessional ? professionalBundle : (isWeek ? selfWeeklySummary : selfPeriodBundle);
  ...
  const metricDays = React.useMemo(() => {
    const rawDays = isWeek && !isProfessional ? (Array.isArray(bundleData) ? bundleData : bundleData?.weekly ?? []) : (bundleData?.daily ?? []);
