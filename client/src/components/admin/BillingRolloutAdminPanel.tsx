...
function phaseLabel(value: Phase | string) {
  return ({ fake: "Provider fake", sandbox: "Sandbox", internal: "Internos", pilot_a: "Piloto A", pilot_b: "Piloto B", general_non_blocking: "Geral sem bloqueio", enforced_10: "Enforced 10%", enforced_25: "Enforced 25%", enforced_50: "Enforced 50%", enforced_100: "Enforced 100%" } as Record<string, string>)[value] ?? value;
}
...
export default function BillingRolloutAdminPanel() {
  ...
  const createSnapshot = trpc.billing.adminCreateRolloutSnapshot.useMutation({
    onSuccess: async () => { toast.success("Snapshot determinístico congelado."); await refresh(); },
  ...
  const rollback = trpc.billing.adminRecordRolloutRollback.useMutation({
    onSuccess: async () => { toast.success("Rollback administrativo para open_access registrado."); await refresh(); },
    onError: error => toast.error(error.message),
  });
  const recordGate = trpc.billing.adminRecordRolloutGateDecision.useMutation({
    onSuccess: async () => { toast.success("Decisão de gate registrada."); await refresh(); },
    onError: error => toast.error(error.message),
  });
  const recordIncident = trpc.billing.adminRecordRolloutIncident.useMutation({
    onSuccess: async () => { toast.success("Incidente de rollout registrado."); setIncidentId(""); setIncidentCause(""); setIncidentImpact(""); await refresh(); },
    onError: error => toast.error(error.message),
  });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5" />Rollout comercial</CardTitle>
        <CardDescription>
          Control plane da #898: congela coortes, registra pausas e rollback e mantém a progressão sempre manual. Nenhuma ação desta seção cria cobrança ou assinatura.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {overview.isError ? <div role="alert" className="rounded-xl border border-destructive/30 p-4 text-sm text-destructive">Não foi possível consultar o rollout.</div> : null}
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-xl border p-4"><p className="text-xs text-muted-foreground">Modo de acesso</p><p className="mt-1 font-medium">{overview.data?.runtimeAccessMode ?? "—"}</p></div>
          <div className="rounded-xl border p-4"><p className="text-xs text-muted-foreground">Última fase aprovada</p><p className="mt-1 font-medium">{phaseLabel(overview.data?.currentApprovedPhase ?? "fake")}</p></div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-4 rounded-xl border p-4">
            <div className="flex items-center gap-2"><UsersRound className="h-4 w-4" /><h3 className="font-medium">Congelar coorte</h3></div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2"><Label>Fase</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={phase} onChange={event => setPhase(event.target.value as Phase)}>{PHASES.map(item => <option key={item} value={item}>{phaseLabel(item)}</option>)}</select></div>
              ...
              <div className="space-y-2"><Label>Chave do snapshot</Label><Input value={snapshotKey} onChange={event => setSnapshotKey(event.target.value)} placeholder="pilot-a-2026-08" /></div>
            ...
            <div className="space-y-2"><Label>Critério determinístico</Label><Input value={criterion} onChange={event => setCriterion(event.target.value)} placeholder="Usuários elegíveis aprovados para a fase" /></div>
            <div className="space-y-2"><Label>IDs candidatos</Label><Textarea rows={3} value={candidateIds} onChange={event => setCandidateIds(event.target.value)} placeholder="101, 102, 103" /><p className="text-xs text-muted-foreground">{parsedCandidateIds.length} candidatos válidos. A ordenação de entrada não altera a seleção.</p></div>
            <div className="space-y-2"><Label>Motivo</Label><Textarea rows={2} value={reason} onChange={event => setReason(event.target.value)} /></div>
            <Button disabled={createSnapshot.isPending || parsedCandidateIds.length === 0 || snapshotKey.trim().length < 3 || criterion.trim().length < 3 || commonReason.length < 3} onClick={() => createSnapshot.mutate({ phase, snapshotKey: snapshotKey.trim(), ruleVersion: ruleVersion.trim(), criterion: criterion.trim(), candidateUserIds: parsedCandidateIds, percentage, reason: commonReason })}>Congelar snapshot</Button>
          </div>

          <div className="space-y-4 rounded-xl border p-4">
            <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4" /><h3 className="font-medium">Pausa e rollback</h3></div>
            <p className="text-sm text-muted-foreground">Pausa e rollback são append-only e preservam cobranças, assinaturas, estornos, cancelamentos, capacidade e eventos legítimos. A retomada exige confirmação reforçada.</p>
            <div className="space-y-2"><Label>Fase afetada</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={phase} onChange={event => setPhase(event.target.value as Phase)}>{PHASES.map(item => <option key={item} value={item}>{phaseLabel(item)}</option>)}</select></div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" disabled={pause.isPending || commonReason.length < 3} onClick={() => pause.mutate({ phase, paused: true, scope: "all", reason: commonReason, reinforcedConfirmation: false })}><PauseCircle className="h-4 w-4" />Pausar fase</Button>
              <Button variant="outline" disabled={pause.isPending || commonReason.length < 3 || !reinforced} onClick={() => pause.mutate({ phase, paused: false, scope: "all", reason: commonReason, reinforcedConfirmation: reinforced })}>Retomar fase</Button>
              <Button variant="destructive" disabled={rollback.isPending || commonReason.length < 3 || !reinforced} onClick={() => rollback.mutate({ phase, reason: commonReason, pauseCommunications: true, pauseBlocks: true, reinforcedConfirmation: true })}><RotateCcw className="h-4 w-4" />Registrar rollback</Button>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-4 rounded-xl border p-4">
            <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4" /><h3 className="font-medium">Gate manual da etapa</h3></div>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Fase avaliada</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={phase} onChange={event => setPhase(event.target.value as Phase)}>{PHASES.map(item => <option key={item} value={item}>{phaseLabel(item)}</option>)}</select></div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {(Object.keys(owners) as Array<keyof typeof owners>).map(key => <div key={key} className="space-y-2"><Label>{({ product: "Produto", technical: "Técnico", billing: "Billing", support: "Suporte", authorizer: "Administrador autorizador" } as Record<string, string>)[key]}</Label><Input value={owners[key]} onChange={event => setOwners(current => ({ ...current, [key]: event.target.value }))} /></div>)}
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2"><Label>Processados ≤5 min (%)</Label><Input type="number" min={0} max={100} value={metrics.processedWithin5mBps / 100} onChange={event => setMetrics(current => ({ ...current, processedWithin5mBps: Math.round(Number(event.target.value) * 100) }))} /></div>
              <div className="space-y-2"><Label>Reconciliados ≤30 min (%)</Label><Input type="number" min={0} max={100} value={metrics.reconciledWithin30mBps / 100} onChange={event => setMetrics(current => ({ ...current, reconciledWithin30mBps: Math.round(Number(event.target.value) * 100) }))} /></div>
              ...
              <div className="space-y-2"><Label>Notificações internas persistidas (%)</Label><Input type="number" min={0} max={100} value={metrics.internalNotificationsPersistedBps / 100} onChange={event => setMetrics(current => ({ ...current, internalNotificationsPersistedBps: Math.round(Number(event.target.value) * 100) }))} /></div>
            </div>
          ...
          <div className="space-y-4 rounded-xl border p-4">
            <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4" /><h3 className="font-medium">Incidente do rollout</h3></div>
            <div className="grid gap-3 sm:grid-cols-2">
            ...
            {overview.data?.openIncidents.length ? <div className="space-y-2 border-t pt-4">{overview.data.openIncidents.slice(0, 6).map((item, index) => <div key={`${String(item.incidentId)}-${index}`} className="rounded-lg bg-muted/30 p-3 text-sm"><div className="flex items-center justify-between gap-2"><strong>{String(item.incidentId)}</strong><Badge variant={item.severity === "critical" || item.severity === "high" ? "destructive" : "outline"}>{String(item.severity)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{String(item.type)} · {phaseLabel(String(item.phase))}</p></div>)}</div> : null}
          </div>
        </div>

        <div className="space-y-3">
          <h3 className="font-medium">Snapshots recentes</h3>
          {overview.data?.snapshots.length ? <div className="divide-y rounded-xl border">{overview.data.snapshots.slice(0, 8).map((item, index) => <div key={`${String(item.snapshotKey)}-${index}`} className="flex flex-wrap items-center justify-between gap-3 p-3 text-sm"><div><p className="font-medium">{phaseLabel(String(item.phase))} · {String(item.snapshotKey)}</p><p className="text-xs text-muted-foreground">regra {String(item.ruleVersion)} · {Number(item.effectivePopulation ?? 0)}/{Number(item.plannedPopulation ?? 0)} usuários · {formatDate(item.recordedAt as Date | string | null)}</p></div><Badge variant="outline">{Number(item.percentage ?? 0)}%</Badge></div>)}</div> : <p className="text-sm text-muted-foreground">Nenhuma coorte congelada ainda.</p>}
        </div>
      </CardContent>
    </Card>
  );
