...
export default function BillingCatalogAdminPanel() {
  ...
  const publishVersion = trpc.billing.adminPublishCatalogVersion.useMutation({
    onSuccess: async () => { toast.success("Versão publicada para novas contratações."); await refresh(); },
  ...
  const deactivateVersion = trpc.billing.adminDeactivateCatalogVersion.useMutation({
    onSuccess: async () => { toast.success("Versão encerrada para novas contratações."); await refresh(); },
  ...
  const deactivateCoupon = trpc.billing.adminDeactivateCoupon.useMutation({
    onSuccess: async () => { toast.success("Cupom desativado para novos usos."); await refresh(); },
  ...
  const submitVersion = () => {
    ...
    createVersion.mutate({
      productCode: versionProductCode.trim(),
      ...
      coveredBeneficiaryEntitlements: csv(beneficiaryEntitlements),
  ...
  const submitCoupon = () => {
    ...
    if (!couponCode.trim() || !Number.isInteger(discountValue) || discountValue <= 0 || discountValue > 30 || eligibleCycles.length === 0 || couponReason.trim().length < 3) return;
    createCoupon.mutate({
      code: couponCode.trim(),
      discountType: "percentage",
      discountValue,
      currency: null,
      eligibleProductCodes: csv(couponProducts),
      ...
      durationCharges: eligibleCycles.includes("yearly") ? 1 : Math.min(3, 1),
      active: true,
  ...
  return (
    <section className="space-y-6" aria-labelledby="billing-catalog-admin-title">
              ...
              {versions.data?.map(version => (
                <article key={version.id} className="rounded-xl border p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    {version.status === "draft" ? <Button size="sm" variant="outline" onClick={() => publishVersion.mutate({ versionCode: version.versionCode, effectiveFrom: new Date(), reason: "Publicação administrativa confirmada", provenance: { origin: "admin_manual" } })}>Publicar</Button> : null}
                    {version.status === "active" ? <Button size="sm" variant="outline" onClick={() => deactivateVersion.mutate({ versionCode: version.versionCode, effectiveUntil: new Date(), reason: "Encerramento administrativo para novas contratações" })}>Encerrar</Button> : null}
                  </div>
                </article>
              ))}
            </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle>Novo cupom percentual</CardTitle><CardDescription>Cria uma revisão nova; cobranças confirmadas não são alteradas retroativamente.</CardDescription></CardHeader><CardContent className="space-y-3"><TextField label="Código do cupom" value={couponCode} onChange={setCouponCode} /><TextField label="Percentual" value={couponPercent} onChange={setCouponPercent} /><TextField label="Ciclos" value={couponCycles} onChange={setCouponCycles} /><TextField label="Produtos elegíveis" value={couponProducts} onChange={setCouponProducts} /><Reason value={couponReason} onChange={setCouponReason} id="coupon-reason" /><Button className="w-full" disabled={createCoupon.isPending} onClick={submitCoupon}>Criar revisão</Button></CardContent></Card>
      </div>
