...
| IA multi-provider, benchmark, rollout e fallback por capacidade (#917/#927/#960) | `docs/benchmarks/multi-provider/README.md`, `docs/runbooks/multi-provider-rollout.md`, `docs/runbooks/ai-legacy-config-removal-960.md`, `ARCHITECTURE.md` (seção "Fundação multi-provider de IA"), `docs/design-docs/nutrition-engine.md`, `docs/PRIVACY_LGPD.md`, `docs/SECURITY.md`, `docs/RELIABILITY.md` |
...
- A selecao de provider, modelo, timeout, tentativas e fallback de IA e feita exclusivamente por capacidade em `server/_core/ai/`, usando `AI_<CAPABILITY>_*` e os defaults versionados do resolvedor. Nao reintroduza seletores globais ou aliases de modelo aposentados na #960. Novas capacidades ou adapters devem ser declarados na fundacao comum e consumidores devem reutilizar `resolveCapabilityConfig` + `executeResolvedCapability`, sem selector helper paralelo.
- `OPENAI_API_KEY`, `GEMINI_API_KEY` e `OPENAI_BASE_URL` sao credencial/endpoint dos adapters e nao devem ser confundidos com os seletores aposentados.
