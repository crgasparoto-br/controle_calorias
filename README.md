...
- **matriz baseada no adapter real**: cada provider declara somente operações implementadas no projeto; Gemini declara texto, visão, Structured Output e pesquisa web (Google Search Grounding, #923), sem anunciar embeddings antes de existir método dedicado — por isso `EMBEDDING` preserva `text-embedding-3-small` da OpenAI como default e não tem fallback cross-provider disponível;
