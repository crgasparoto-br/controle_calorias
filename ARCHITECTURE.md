...
- `capabilities.ts` registra `MEAL_TEXT`, `MEAL_VISION`, `WHATSAPP_INTENT`, `QUESTION`, `NUTRITION_SEARCH`, `EMBEDDING`, `TRANSCRIPTION`, `IMAGE_ANNOTATION` e `FOOD_CLASSIFICATION`. `NUTRITION_SEARCH` requer texto, Structured Output e pesquisa web; `EMBEDDING` é uma capacidade separada. `FOOD_CLASSIFICATION` permanece reservada, pois a NOVA continua embutida em `MEAL_TEXT`/`MEAL_VISION` nesta fase.
- `supportMatrix.ts` declara operações realmente implementadas pelos adapters. OpenAI declara o conjunto completo; Gemini declara somente texto, visão e Structured Output nesta fase. Embeddings Gemini permanecem indisponíveis até existir método dedicado no adapter e teste correspondente.
...
- `policyExecutor.ts` classifica erros concretos de SDK/HTTP/rede e valida saídas vazias, JSON inválido e payload inválido. Cada chamada recebe `AbortSignal`; depois de timeout, retry/fallback só começa após a chamada anterior encerrar. Se o provider não confirmar cancelamento dentro da janela, a execução termina fail-closed sem nova chamada. `MAX_ATTEMPTS` conta todas as chamadas do primário e existe no máximo uma chamada posterior de fallback.
