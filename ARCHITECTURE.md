...
`server/_core/ai/` é a fundação compartilhada para evoluir a seleção de IA de global (`AI_VISION_PROVIDER`) para configuração independente por capacidade. Esta issue cria a fundação e migra somente o SDK do Gemini; nenhum consumidor existente foi migrado para o resolvedor.

- `capabilities.ts` registra `MEAL_TEXT`, `MEAL_VISION`, `WHATSAPP_INTENT`, `QUESTION`, `NUTRITION_SEARCH`, `EMBEDDING`, `TRANSCRIPTION`, `IMAGE_ANNOTATION` e `FOOD_CLASSIFICATION`. `QUESTION` exige texto e pesquisa web porque o consumidor legado do WhatsApp sempre envia `web_search_preview`; `NUTRITION_SEARCH` exige texto, Structured Output e pesquisa web; `EMBEDDING` é uma capacidade separada com consumidor legado direto. `FOOD_CLASSIFICATION` permanece reservada, pois a NOVA continua embutida em `MEAL_TEXT`/`MEAL_VISION` nesta fase.
- `supportMatrix.ts` declara somente operações implementadas pelo adapter do projeto. No OpenAI, texto/visão/Structured Output/pesquisa web passam por `createTextResponse`, embeddings por `createEmbeddings`, transcrição por `createAudioTranscription` e geração/edição por `createImageGeneration`; cada família possui tradução local e teste do adapter. Gemini declara somente texto, visão e Structured Output nesta fase. Pesquisa web, embeddings, transcrição e imagem Gemini permanecem inelegíveis até existir método/tradução e teste correspondente.
...
- `policyExecutor.ts` recebe a política resolvida completa. Estados `disabled` e `invalid`, limites não inteiros/positivos e fallback habilitado sem callback são rejeitados antes de qualquer callback ou outbound. Estados `ready` e `degraded` são executáveis somente quando o primário permanece válido.
...
- Requests comuns são fail-closed: recurso recebido pelo adapter deve ser traduzido ou rejeitado antes da rede. O Gemini rejeita `tools` enquanto não existir tradução explícita da ferramenta interna para Google Search; não descarta ferramentas silenciosamente.
...
**Compatibilidade legada**: `ENV.visionModel`, `AI_VISION_PROVIDER`, `GEMINI_MODEL`, `OPENAI_MODEL` e demais variáveis legadas continuam disponíveis. O resolvedor inclui aviso `[deprecated]` sanitizado em `diagnostics` quando usa uma variável legada. Como nenhum consumidor chama o resolvedor nesta subissue, esse diagnóstico ainda não é emitido como log de aplicação.
