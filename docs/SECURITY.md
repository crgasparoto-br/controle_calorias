...
- Na web, `read_only_access` bloqueia mutations protegidas por padrão. As únicas exceções profissionais de escrita são uma allowlist exata para `professionalRecord.settings.updateIdentity`, `professionalRecord.settings.updatePreferences` e `professionalRecord.settings.setActive`; nesta última, somente a desativação (`active=false`) permanece disponível durante suspensão, enquanto reativação, ações clínicas, carteira, pacientes e demais recursos pagos continuam negados pelo guard de domínio.
- Logs devem ser úteis para operação, mas sanitizados para dados sensíveis.
- Rotas públicas de governança de uso, inclusive aliases em billing, passam pelo mesmo tradutor de erro; SQL, host, schema, payload, stack e mensagem interna nunca atravessam a fronteira tRPC.
...
- Somente um vínculo ativo pode possuir cada telefone normalizado. A migration preserva históricos desativados, mantém o registro ativo mais recente em duplicidades legadas e instala índice único sobre a chave virtual dos vínculos ativos.
