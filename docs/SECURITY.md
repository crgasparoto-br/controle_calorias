...
O benchmark de #924 é executado localmente em ambiente autorizado e não deve motivar workflow novo, job de PR, rerun manual ou exposição de `OPENAI_API_KEY` a código não mergeado. O workflow de PR mantém somente testes herméticos; somente o JSON sanitizado descrito em `docs/benchmarks/transcription/results/README.md` pode ser versionado.
