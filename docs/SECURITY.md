...
O workflow temporário da issue #922 foi aposentado depois da validação daquela entrega. Testes versionados não devem depender de workflows temporários já removidos. Para #923, `scripts/issue-923-live-provider-smoke.ts` fornece um harness que valida `QUESTION` em modo sem busca, `QUESTION` com pesquisa real, `NUTRITION_SEARCH` com fonte citada em uma única tentativa governada pelo executor e `EMBEDDING` com vetor real. O workflow usa o ambiente protegido `issue-923-live-smoke`, exige a variável externa `AI_SMOKE_APPROVED_SHA` igual ao `HEAD`, desabilita persistência de credenciais no checkout e libera somente `AI_SMOKE_OPENAI_API_KEY`/`AI_SMOKE_GEMINI_API_KEY` no passo final. O harness permite modelos separados por `SMOKE_QUESTION_MODEL` e `SMOKE_NUTRITION_MODEL`; no Gemini, a pesquisa nutricional requer Gemini 3, enquanto perguntas podem continuar em Gemini 2.5. As credenciais devem ser dedicadas, de quota mínima e nunca reutilizar segredos comuns de produção. Ausência de ambiente aprovado, SHA aprovado ou credencial dedicada deve falhar fechado e ser registrada como limitação, não como smoke aprovado.
