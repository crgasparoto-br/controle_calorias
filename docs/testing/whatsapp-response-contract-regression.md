...
|---|---|---|---|---|
| Caso de produção composto | `1 banana nanica, 1 pêra packans`, com uma referência verificável `same_food_type` para `1 unidade de pêra` | entrypoint público `handleWhatsAppWebhookWithTextIntent` → preflight de identidade comercial → `prepareCountableFoodRegistrationResolved` → `resolveHouseholdMeasure` | `packans` permanece qualificador natural não comprovado; não vira marca; a banana continua canônica e a pêra segue para `contextual_estimate` sem clarificação de peso | `whatsappIntentWebhook.issue1181.test.ts`, `commercialFoodIdentityPreflight.issue1181.test.ts`, `householdMeasureResolution.issue1181.test.ts` |
| Caso de produção com múltiplas unidades | `1 banana nanica, 6 uvas pretas, 1 pêra packans` | mesmo entrypoint público; a referência natural canônica amplia apenas a pesquisa de quantidade e o resolvedor real multiplica a relação unidade → gramas | quando as três medidas são resolvidas, `6 uvas pretas` preserva multiplicador 6 e identidade original; se uma medida ficar pendente, os demais itens resolvidos são registrados e a pendência é listada sem assumir 100 g | `whatsappIntentWebhook.issue1181.test.ts`, `householdMeasureResolution.issue1181.test.ts` |
