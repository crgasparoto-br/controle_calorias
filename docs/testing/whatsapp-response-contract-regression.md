...
| Comando isolado sem pendência (`registrar`, `confirmar`, `cancelar`, `editar`, `consultar`, `sim`, `não`, `ok`, número solto) | `1 iogurte natual desnatado` → clarificação genérica → `registrar` | `executeWhatsappLlmIntent` bloqueia a palavra isolada antes de qualquer chamada ao LLM quando não há pendência ativa; mesma função usada pelo webhook real e pelo simulador | nenhum — não alcança `processMealDraft`/`processMealInput`, não cria alimento | resposta central de esclarecimento ("Não encontrei uma operação pendente...") | 1 mensagem | nunca criar item fantasma a partir de comando isolado; frases completas como `registrar 100 g de arroz` continuam registrando normalmente | `llmIntentActions.test.ts`, `intentInterpreter.test.ts`, `intentRouter.test.ts`, `service.test.ts`, `standaloneCommandWords.test.ts` | concluído (#855); clarificação genérica ambígua agora é decisão fechada com pendência persistente (#858) |
| Clarificação genérica ambígua (decisão fechada) | mensagem sem alimento/comando claro → lista `Registrar alimento` / `Corrigir refeição` / `Consultar registros` / `Cancelar` | precedence gate + `intentClarificationInteraction.ts` + `whatsappPendingOperations` (`intent_clarification`) | nenhum efeito de domínio até a escolha; **Registrar alimento** nunca persiste a palavra de comando como alimento (vira pergunta aberta); **Consultar registros** reutiliza o serviço de resumos | `buildWhatsappIntentClarificationReply` (lista, 4 ações) | 1 pergunta (lista) + 1 resposta | preservar a mensagem original; resposta inválida reapresenta a mesma lista sem nova pendência | `intentClarificationInteraction.test.ts`, `interactionReplay.test.ts`, `messageRouter.test.ts` | concluído (#858) |
| Reclassificação ambígua (decisão de escopo) | registros recentes com classificações diferentes → botões `Só compatíveis` / `Todos recentes` / `Cancelar` | `webhookTextCommands.ts` (`handleWhatsAppAction`) + `whatsappPendingOperations` (`confirmation`, `decision: reclassify_scope`) | reclassifica apenas o escopo escolhido; `sim` isolado é ambíguo entre os dois escopos e reapresenta em vez de decidir pelo usuário | `buildWhatsappClosedDecisionReply` (botões, 3 ações) | 1 pergunta (botões) + 1 confirmação | decisão fechada com Cancelar incluído; texto `APENAS`/`TODOS`/`CANCELAR` resolve a mesma pendência do botão | `webhookTextCommands.test.ts` | concluído (#858) |
| Resposta inválida a pendência ativa (reapresentação transversal) | índice fora da faixa, palavra de comando isolada ou confirmação incompatível durante exclusão, seleção, confirmação genérica, período ou clarificação genérica | precedence gate → `interactionReplay.ts` (`classifyWhatsappPendingTextResponse`/`rebuildWhatsappPendingInteractionReply`) | nenhum — não consome a pendência, não muta, não alcança parser/LLM/fallback nutricional | reconstrução a partir do `target` persistido, mesmas ações/ordem/rótulos, callbacks renovados para a mesma pendência | 1 mensagem (mesma interação reapresentada) | pendência expirada/consumida responde indisponibilidade sem recriar silenciosamente; comando completo novo segue o fluxo normal sem consumir a pendência | `interactionReplay.test.ts`, `messageRouter.test.ts`, `mealItemSelectionCallback` (via `messageRouter.interactiveCallback.test.ts`) | concluído (#858) |
| Inventário transversal de decisões fechadas | qualquer tipo de pendência resolvível pelo gate central | `interactionInventory.ts` (fonte única, versionada) + `WHATSAPP_REGISTERED_PENDING_TYPES` em `messageRouter.ts` | nenhum efeito de domínio — apenas contrato/registro | `buildWhatsappClosedDecisionReply` centraliza a escolha botões×lista (até 3 ações → botões; 4+ → lista, contando Cancelar) | n/a | todo tipo novo de pendência fechada exige entrada no inventário, sob pena de falha em teste automatizado | `interactionInventory.test.ts` | concluído (#858) |
