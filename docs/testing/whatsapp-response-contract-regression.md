...
**Status parcial (nesta entrega):** a consulta de refeição/alimentos (`mealListIntent.ts`) foi migrada para reutilizar os blocos centrais de item e total de `replyTemplates.ts` (`buildWhatsAppFoodLines`/`buildWhatsAppMealTotalLines`), os mesmos usados pelo registro em `replyMessages.ts`. Os demais fluxos da tabela (adição, substituição, ajuste de quantidade, exclusão, alimento estimado) **ainda não foram migrados** nesta entrega e permanecem com builders locais — ver PR para escopo e justificativa da divisão em partes menores.
...
|---|---|---|---|---|---|---|---|---|
| Registro por texto (fallback nutricional) | `100g arroz branco` | idempotency → intent (não trata) → annotated (sem imagem) → base | 1 refeição criada + consolidação | `buildWhatsAppMealReplyMessage`/`buildWhatsAppConsolidatedMealReplyMessage` + botão quick edit | ack de texto + 1 resposta funcional | padrão único de refeição, sem ack no caminho rápido | `whatsappWebhook.test.ts`, `whatsappPersistentContextWebhook.test.ts` | ordem/quantidade física e ack fora do lifecycle — coberto pela baseline |
| Adição a refeição existente (intent) | `adiciona 1 banana no almoço` | intent webhook (`executeWhatsappTextIntent` → `sendAndLogTextReply`) | item adicionado 1x | reply do intent + `buildMealFullSummary` local (formato legado) + quick edit | 1 mensagem | repetir todos os itens e totais no formato central | `intentActions.*.test.ts`, `whatsappIntentWebhook.test.ts` | builder local duplica formato de refeição — pendente |
| Listagem de refeição | `o que registrei no almoço?` | intent webhook (`mealListIntent`) | nenhum (consulta ao banco) | ✅ migrado: `mealListIntent.ts` usa `buildWhatsAppFoodLines`/`buildWhatsAppMealTotalLines` (mesmo bloco do registro) | 1 mensagem | mesmo padrão visual do registro | `mealListIntent.test.ts`, `whatsappIntentWebhook.mealList.test.ts` | concluído nesta entrega |
| Substituição contextual | `não era arroz, era batata` | intent webhook (`contextualFoodReplacementIntent`) | substituição aplicada direto (sem confirmação) | reply próprio | 1 mensagem | aplicar direto + repetir itens/totais | `contextualFoodReplacementIntent.test.ts` | formato próprio |
| Ajuste de gramas/quantidade | `muda o arroz para 150g` | intent webhook (`gramsAdjustmentIntent`/`gramsIncrementIntent`) | update no registro correto | reply próprio | 1 mensagem | aplicar direto + repetir itens/totais | `gramsAdjustmentIntent*.test.ts`, `gramsIncrementIntent.test.ts` | formato próprio |
| Exclusão (alimento/refeição) | `exclua o almoço` → botão `Confirmar` (ou texto `sim`) | precedence gate + `deleteIntent` + `interactiveCallback` + `whatsappPendingOperations` | exclusão só após confirmação; consumo compare-and-set (botão ou texto resolvem a mesma pendência) | botões `Confirmar`/`Cancelar` (issue #782) com fallback textual `sim`/`cancelar` mantido | 1 pergunta (botões) + 1 confirmação | ✅ migrado (#782); resposta central segue evoluindo nas próximas subissues de formato | `deleteIntent*.test.ts`, `whatsappIntentWebhook.delete.test.ts`, `whatsappIntentWebhook.interactiveCallback.test.ts`, `messageRouter.interactiveCallback.test.ts` | formato textual da pergunta ainda não usa os blocos finais de #781 (título/ícone), só o transporte por botões |
| Exclusão via legenda de imagem | foto com legenda `apague o almoço` | annotated webhook desvia para `deleteIntent` | nenhuma refeição criada da imagem | reply do deleteIntent via texto simples | ack + 1 resposta | sem duplicidade, resposta central | `whatsappWebhook.mediaIntent.test.ts` | ack antes de desvio de intenção |
| Alimento ausente do catálogo | `1 bisnaguinha xpto` | intent webhook (`buildUnknownFoodReply`) ou estimativa IA | nenhum registro (aviso) ou item estimado | texto fixo local / `replyMessages.estimatedNutrition` | 1 mensagem | seguir direto para estimativa da IA; aviso abaixo do item estimado | `whatsappUnknownFoodWebhook.test.ts`, `replyMessages.estimatedNutrition.test.ts` | decisão de produto muda o fluxo atual |
