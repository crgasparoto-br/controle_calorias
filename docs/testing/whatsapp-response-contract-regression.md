...
| Adição a refeição existente (intent) | `adiciona 1 banana no almoço` | intent webhook (`executeWhatsappTextIntent` → `sendAndLogTextReply`) | item adicionado 1x | reply do intent + `buildMealFullSummary` local (formato legado) + quick edit | 1 mensagem | repetir todos os itens e totais no formato central | `intentActions.*.test.ts`, `whatsappIntentWebhook.test.ts` | builder local duplica formato de refeição |
| Listagem de refeição | `o que registrei no almoço?` | intent webhook (`mealListIntent`) | nenhum (consulta ao banco) | reply próprio do intent | 1 mensagem | mesmo padrão visual do registro | `mealListIntent.test.ts`, `whatsappIntentWebhook.mealList.test.ts` | dois formatos para a mesma refeição |
...
| Exclusão (alimento/refeição) | `exclua o almoço` → `Confirmar` | precedence gate + `deleteIntent` + `whatsappPendingOperations` | exclusão só após confirmação; consumo compare-and-set | `buildWhatsAppActionConfirmationRequestReplyMessage` e afins | 1 pergunta + 1 confirmação | botões `Confirmar`/`Cancelar` (#782) e resposta central | `deleteIntent*.test.ts`, `whatsappIntentWebhook.delete.test.ts`, `messageRouter.test.ts` | confirmação por texto, não por botões |
...
| Pedido de acesso profissional | profissional solicita acesso (web) | `professionals/service` notifica paciente | pendência de autorização | texto local | 1 mensagem proativa | botões `Autorizar`/`Recusar` (#782) | `professionalPatientFlow.test.ts` | decisão hoje por texto (`autorizo`/`nego`) |
| Decisão do paciente | `autorizo` | intent webhook (`looksLikeProfessionalAccessDecision`) | autorização aplicada 1x | reply do serviço de profissionais | 1 mensagem | resposta central; sem expor IDs | `professionalPatientFlow.test.ts` | matcher textual amplo pode capturar outros textos |
...
| `whatsappPendingOperations` | confirmação genérica, seleção de exclusão, clarificação de período | reutilizar para botões/listas/callbacks com validação de usuário, expiração e consumo idempotente — #782 |
