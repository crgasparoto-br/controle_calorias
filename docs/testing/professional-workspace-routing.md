...
Os testes funcionais da visão agregada devem degradar isoladamente `activity`, `schedule` e `tracking`: ao falhar um bloco, os outros permanecem renderizados e somente o retry daquele bloco é acionado. Fixtures discriminantes devem retornar `null` para um indicador indisponível e `0` para outro indicador calculado sem ocorrências, comprovando que a interface não transforma ausência em zero. O teste de repositório também confirma uma consulta SQL por bloco, escopo pelo profissional autenticado e janela UTC indexável com classificação no timezone do paciente apenas para `activity`.
