...
| Substituições contextuais multiline (#918) | `Não é requeijão, é maionese.\nNão é presunto, é mortadela defumada` | webhook ou simulador → handler canônico `contextualFoodReplacementIntent` | todas as correções válidas formam um único lote; segmento incompleto bloqueia tudo antes da consulta/mutação | seleção persistida quando ambíguo, `mealBatchMutation` para escrita/compensação e resposta canônica do estado recarregado | 1 resposta funcional lógica | preservar cada origem → destino; aceitar `LF`/`CRLF`/linhas em branco; sem fallback ou refeição nova | `contextualFoodReplacementIntent.multiline.test.ts` + smoke de entrypoints | concluído (#918) |
...
| Alimento ausente do catálogo / estimado pela IA | `1 bisnaguinha xpto` | intent webhook / fluxo de registro segue para estimativa da IA | item estimado incluído na refeição e nos totais | `buildWhatsAppFoodLines` mantém calorias e macros estimados sem `WHATSAPP_ESTIMATED_NUTRITION_WARNING` visual | 1 mensagem | seguir direto para estimativa da IA; sem aviso individual; totais incluem estimados | `replyMessages.estimatedNutrition.test.ts`, `whatsappIntentWebhook.test.ts` | concluído |
...
| Notificação de exercício (Strava) | activity webhook Strava | `strava/exercises.ts` | exercício upserted; reimportação não duplica calorias | `buildWhatsAppCanonicalExerciseReply` + CTA, com `goalProgress` do dia via `getWhatsAppMealGoalProgress` | 1 mensagem proativa | contrato central; inclui Meta, Exercícios, Consumo e exatamente um saldo entre Superávit, Déficit ou Equilíbrio (`buildWhatsAppGoalProgressLines`); Exercícios permanece informativo e a mensagem não infere se alterou a meta efetiva | `modules/healthIntegrations/strava/*.test.ts`, `whatsappImageIdempotencyWebhook.test.ts` | concluído (#787) |
