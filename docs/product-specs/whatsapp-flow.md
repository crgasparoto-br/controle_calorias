...
- Pedidos de assistente alimentar devem responder com orientação educativa e não devem criar refeição automaticamente.

## Entradas suportadas

| Tipo | Comportamento esperado |
|---|---|
| Texto | Processar descrição livre da refeição ou responder intenções de texto, incluindo assistente alimentar |
...
- Pedidos como `Assistente alimentar, o que posso comer no jantar?` respondem pelo WhatsApp sem cair no fallback de registro de refeição.
