...
- A assinatura `150 kcal / 100 g`, `6 g` de proteína, `15 g` de carboidratos e `5 g` de gorduras é um placeholder de inferência, não uma evidência nutricional. O motor deve tentar a referência local/TACO ou pesquisa verificada antes de escalar esse perfil; se a identidade comercial estiver explícita e a fonte não for verificável, deve manter o fluxo fail-closed de clarificação.
...
| Contrato semântico                            | `buildMealSemanticContract` no domínio nutricional                                                         | `materializeResolvedCommercialMeal` pode construir um resultado transitório a partir de decisão estruturada                                  | o gate não pode inferir novamente marca, variante, quantidade ou origem                   |
...
A preparação de medidas contáveis tem um único owner assíncrono em `prepareCountableFoodRegistrationResolved`. O export histórico `prepareCountableFoodRegistration` é apenas um adaptador local síncrono para a suíte de compatibilidade #997 e eventuais consumidores externos; não há consumidor produtivo interno e ele não contém uma segunda decisão de parsing/porção. Sua condição objetiva de aposentadoria é uma varredura de consumidores do repositório/package com zero chamadas síncronas restantes, seguida da remoção do export e da suíte de compatibilidade migrada.

A identidade textual estruturada tem como owner `resolveStructuredCommercialIdentity`. O único consumidor produtivo interno da API anterior `recoverCanonicalCommercialIdentity` foi migrado para esse owner na resolução canônica de adição; o adaptador assíncrono permanece somente para integrações externas que dependam da assinatura histórica. Sua condição objetiva de aposentadoria é confirmar zero consumidores fora deste repositório/package, remover o segundo parâmetro legado e retirar o export após executar a matriz #1094/#1016.
...
## Revalidação da Fase 3 — issue #1096

A Fase 3 foi iniciada sobre o SHA-base de `develop` `b3156bcf28cf854b14a657f4750a3f78b2a5ac67` e o commit de implementação auditado é `24cac382d9aa9e5b0c7a5f79dd94c70f10d56728`, com follow-up documental `a95a5c54c290f7a6aeffe7bb3b1bb892b5b560c3`; a referência da Fase 0 é o SHA `4d86cb814ff57164ff16cd7626ea21be46719fce`. A revalidação independente foi executada contra `develop@394337d1166c4d12df3c78010246f65a63037eee`, capturado em `2026-09-17T13:51:57Z` por `git rev-parse origin/develop` antes desta correção. Antes da decisão, a composição do POST público foi revalidada com a caracterização da #1094, testes runtime de delegação e a auditoria de reachability verificou a cadeia `registerWhatsAppPublicPostRoute -> handleWhatsAppPersistentContextWebhook -> handleWhatsAppWebhookWithImageIdempotency -> handleWhatsAppWebhookWithTextIntent -> handleWhatsAppWebhookWithAnnotatedImages -> handleWhatsAppWebhook -> handleWhatsAppWebhookImplementation`.

Os findings foram mantidos com as seguintes disposições: **F0-05 = `keep`**, porque `server/whatsappWebhook.ts` continua sendo fachada de compatibilidade, verificação pública e fallback produtivo; **F0-06 = `defer`**, porque os wrappers ainda preservam gates distintos de lifecycle, precedência, idempotência, intenção e mídia; e **F0-08 = `defer`**, porque os candidatos restantes possuem reachability ou consumidores históricos comprovados. Nenhuma fachada ou bridge foi removido por similaridade textual.

O inventário de owners, consumidores, substitutos, golden flows e condições objetivas de aposentadoria está em `docs/testing/issue-1096-bridge-reachability.md`. A Fase 3 não alterou código de produção; portanto, seu delta operacional é zero para `processMealInput`, `NUTRITION_SEARCH`, persistências, claims e round-trips. A caracterização pública pós-implementação permaneceu verde, e a regressão explícita de #1072 foi reexecutada com 30/30 testes aprovados. Os valores por cenário e a comparação operacional before/after estão em `docs/testing/issue-1096-metrics.md`, e o parecer independente está em `docs/testing/issue-1096-independent-audit.md`. A documentação não autoriza a remoção dos adaptadores históricos `recoverCanonicalCommercialIdentity` e `prepareCountableFoodRegistration` enquanto as condições de aposentadoria registradas não forem satisfeitas.

## Validação final da Fase 4 — issue #1097

A validação final é executada por `pnpm issue-1097:validate`. O runner reexecuta a caracterização pública da #1094, valida schema, issue, entrypoint, baselines e todos os cenários obrigatórios, confirma a ancestralidade das fases #1094/#1095/#1096 e executa as regressões específicas e os gates `check`, `test`, `architecture:check`, `docs:check`, `agent:check` e `build`; as suítes unitárias/in-memory recebem `DATABASE_URL` vazio para não acessar a base externa, e `db:check-integrity` é executado separadamente quando `DATABASE_URL` está disponível. Sem `DATABASE_URL`, o relatório registra `skipped-no-database-url` e permanece **não aprovado**, nunca sendo tratado como sucesso. A caracterização deve preservar os limites da baseline: no máximo uma tentativa e uma operação outbound de `NUTRITION_SEARCH` por item comercial, sem falhas de round-trip, violações de monotonicidade ou mutações parciais.

O smoke publicado é `pnpm smoke:issue-1097:post-deploy`. Ele exige uma identidade de teste já autorizada, não altera catálogo ou cache e envia os casos Panco e comercial não-Panco pelo `POST /api/whatsapp/webhook`, exigindo `processed=1` e o outcome fixo `meal_registered` na primeira chamada, e repetindo a mesma `message.id` para exigir `processed=0`, `deduplicated=true` e `duplicate_ignored` no replay. O runtime fornece `x-runtime-commit` somente quando `RENDER_GIT_COMMIT` contém o SHA hexadecimal completo de 40 caracteres, e o smoke compara esse valor por igualdade exata com o candidato; `x-whatsapp-processing-outcome` usa somente uma enumeração fixa. Nenhum dos headers inclui conteúdo alimentar, telefone, URL, prompt, resposta de provider ou erro bruto. O workflow `.github/workflows/issue-1097-validation.yml` vincula os gates ao SHA do workflow, publica os relatórios JSON e oferece execução manual do smoke pós-deploy.

A revalidação da #1097 também confirmou o snapshot corrente `develop@99686ce218745b34e347d44240711e1d51a4d4fd`, capturado em `2026-09-17T19:04:12Z` por `git rev-parse origin/develop`; a referência histórica da Fase 3 permanece preservada e não altera os owners ou as disposições de reachability.

## Memória, feedback e observabilidade governados (#1153)

A memória pessoal de alimentos e medidas usa os owners existentes e persiste seus artefatos em `whatsappLearningArtifacts`, com escopo `user` ou `global`. A chave inclui a identidade alimentar completa; portanto, nome, marca, variante, contexto e porção não são colapsados em um alias simples. O resolvedor lê a fonte persistente antes da decisão e falha fechado quando ela está indisponível. A entrada explícita atual tem precedência sobre qualquer memória aplicável.

Histórico de mensagens, feedback, review queue, candidatos e relatórios de métricas carregam referências sanitizadas ao histórico e ao item alimentar. Observações de itens separam marca, variante, quantidade/medida, fonte, confiança, ambiguidade, correção posterior, fallback e procedência de OCR/transcrição. A ocorrência de fallback (`fallbackRate`) é distinta de segurança classificada do fallback (`fallbackSafeRate`); na ausência de classificação, o denominador de segurança permanece explícito e não representa aprovação automática.
