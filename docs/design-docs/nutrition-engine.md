...
- Para imagem com `brand` estruturada, o motor monta uma única identidade comercial mais específica com alimento, marca, variação e `portionText`, tenta catálogo exato e depois `NUTRITION_SEARCH` antes de admitir o candidato genérico. O resultado específico precisa devolver a mesma marca e passar pelos guards de variante e embalagem; marca divergente ou fonte sem evidência degrada sem nova consulta externa.
- A marca estruturada é recomposta no `foodName` exibido quando ainda não aparece no nome reconhecido, sem duplicação. `brand` continua separado e o `canonicalName` registra a referência efetivamente usada; assim um fallback genérico preserva a identidade visual, mas permanece `heuristic`/`hybrid` em vez de parecer fonte específica da marca.
- Itens marcados por `MEAL_VISION` como `isPlainWater=true` não ativam pesquisa nutricional específica por causa da marca. O split canônico do WhatsApp descarta qualquer referência nutricional de água pura antes de persistir refeição ou responder.
...
- Bebidas: a mesma validação aceita porções rastreáveis em `ml`, além de massa em `g`, e mantém `Original`, `Pilsen`, `Lager`, `Weissbier`, `Zero` e qualificadores equivalentes como termos discriminantes. Uma busca específica por item continua sendo uma única operação governada; resultado funcional ausente ou incompatível não provoca consultas sucessivas com identidades progressivamente mais genéricas.
