...
A validação final é executada por `pnpm issue-1097:validate`. O runner reexecuta a caracterização pública da #1094, as regressões específicas das fases #1095/#1096 e os gates `check`, `test`, `architecture:check`, `docs:check`, `agent:check` e `build`; `db:check-integrity` é executado somente quando `DATABASE_URL` está disponível e sua ausência é registrada como indisponibilidade, nunca como sucesso. A caracterização deve preservar os limites da baseline: no máximo uma tentativa e uma operação outbound de `NUTRITION_SEARCH` por item comercial, sem falhas de round-trip, violações de monotonicidade ou mutações parciais.

O smoke publicado é `pnpm smoke:issue-1097:post-deploy`. Ele exige uma identidade de teste já autorizada, não altera catálogo ou cache e envia os casos Panco e comercial não-Panco pelo `POST /api/whatsapp/webhook`, repetindo a mesma `message.id` para verificar idempotência. O script só imprime status, contagem sanitizada, rótulo do caso e `x-runtime-commit`; o commit esperado deve ser o candidato ou um descendente que o contenha integralmente. O runtime fornece esse cabeçalho a partir de `RENDER_GIT_COMMIT`, limitado a um prefixo hexadecimal, sem incluir conteúdo alimentar, telefone, URL, prompt, resposta ou erro bruto.
