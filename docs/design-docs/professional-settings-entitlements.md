...
O endpoint pessoal `nutrition.professionals.upsertProfile`, usado na ativação inicial, aplica a mesma garantia: se o perfil canônico for gravado e o evento `profile_upserted` falhar, o perfil anterior é restaurado ou o perfil recém-criado é removido antes de propagar o erro. Se a compensação falhar, a API retorna erro explícito de consistência. No cliente, sucesso e erro refazem sessão, perfil e entitlements para que a navegação represente o estado persistido confirmado, inclusive após falhas temporárias.
