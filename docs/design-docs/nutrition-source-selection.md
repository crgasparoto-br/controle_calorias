...
A proxima etapa deve plugar esse seletor no fluxo de montagem do rascunho alimentar, hoje concentrado em `server/nutritionEngine.ts`, para que cada `MealDraftItem` carregue metadados de fonte alem do campo legado `source`.
