...
- `normalizedText` e `routerText`: texto seguro para os roteadores atuais, com unidades e linguagem informal normalizadas.
- `transcribedText`: texto transcrito de áudio, quando existir.
- `informalNormalization`: texto original, texto normalizado, matches aplicados, termos incertos e aliases candidatos.
...
- `needsClarification`: indica quando a mídia ou uma inferência informal não tem contexto suficiente para roteamento seguro.

Regras iniciais:

- Texto puro segue para o roteador depois de normalizar unidades e linguagem informal.
- Áudio usa transcrição informada ou provider injetável; sem transcrição, pede esclarecimento.
- Imagem com legenda usa a legenda como contexto de classificação e normalização informal.
...
A issue #427 é representada por `server/modules/whatsapp/informalTextNormalizer.ts` e integrada ao normalizador multimodal.

A camada preserva o texto original e produz uma versão normalizada para roteamento. Ela diferencia substituições seguras de termos incertos que exigem confirmação.

Cobertura inicial:

- acentos e erros comuns: `cafe`, `pao`, `feijao`, `macarrao`, `proteina`;
- abreviações: `pao c queijo`, `qjo`, `refri`, `add`;
- marcas e aliases candidatos: `cafe lor`, `lor`, `miojo turma da monica`, `zero`;
- quantidades informais: `2 fatia`, `um tiquinho de`, `pratao de`;
- comandos comuns: `registra`, `add`.

Regras de segurança:

- `originalText` nunca é sobrescrito.
- `routerText` usa a normalização apenas como entrada de interpretação.
