...
- A mesma resposta produz a projeção mínima e estável do cabeçalho: autorização aprovada, acompanhamento, evento profissional mais recente em `professionalHistoryEvents` e próxima revisão do tracking. Esses campos independem da página atual da timeline e permanecem disponíveis em prontuário, relatórios e mensagens sem introduzir dependência indireta de `professional_record`. Testes usam datas divergentes entre refeição e timeline para proteger a precedência; no estado `ended`, autorização interna, última atividade e próxima revisão não atravessam o contrato público.
...
- Rascunhos de avaliação, anotação e orientação são mantidos temporariamente pelo par `authorizationId`/`patientId`, impedindo reutilização após revogação e nova autorização da mesma pessoa. Em navegadores sem Navigation API, um `popstate` cancelado restaura a URL e sincroniza novamente o roteador antes de remontar a rota, sem perder o conteúdo. Salvar ou descartar explicitamente remove a seção correspondente; revogação, perda do perfil, encerramento da sessão ou desmontagem do shell eliminam os rascunhos do contexto anterior.
...
- Acompanhamento `ended` mantém somente `/history`: a conclusão da mutação atualiza o contexto canônico e redireciona imediatamente para o histórico, sem depender do polling ou de recuperar o foco. As demais rotas individuais redirecionam antes de montar conteúdo. O backend executa somente as consultas da timeline em `professionalRecord.get`; o payload do prontuário omite `authorizationId`, nome, e-mail, avaliações, anotações e orientações. O contexto individual mantém apenas o nome de exibição necessário para identificar o histórico e omite autorização interna, última atividade e próxima revisão. As APIs de timezone, dashboard, relatório de período e conversa individual deixam de retornar dados do paciente.
