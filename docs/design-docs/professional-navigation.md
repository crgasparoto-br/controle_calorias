...
- A mesma resposta produz a projeção mínima do cabeçalho: autorização aprovada, acompanhamento e próxima revisão do tracking são obtidos pela consulta essencial; o resumo textual seguro e o horário do evento profissional mais recente em `professionalHistoryEvents` são metadados complementares. O resumo usa o catálogo canônico do backend e não pode ser substituído apenas pela data; o evento é escolhido por `occurredAt DESC, id DESC` para manter desempate estável. Falha ou timeout desses metadados complementares não bloqueia o contexto autorizado: o cabeçalho mostra **Temporariamente indisponível**, enquanto **Não informado** fica reservado para ausência confirmada de evento. Consultas complementares que ultrapassam o orçamento perdem a propriedade de cache para permitir retry após TTL curto, mas continuam contabilizadas até realmente terminarem; um limitador global inicia no máximo `min(4, connectionLimit - 1)` operações pendentes, preservando ao menos uma conexão para autorização e demais consultas essenciais. Quando esse limite está ocupado, novas leituras degradam sem iniciar outra query e voltam a tentar depois que um slot é liberado. Esses campos independem da página atual da timeline e permanecem disponíveis em prontuário, relatórios e mensagens sem introduzir dependência indireta de `professional_record`. Testes usam datas divergentes entre refeição e timeline para proteger a precedência; no estado `ended`, autorização interna, última atividade e próxima revisão não atravessam o contrato público.
