...
A identidade durável dessa janela é o `windowKey` do fato inicial. A reconstrução do estado corrente localiza esse fato raiz explicitamente e resolve encerramento/extensões pela mesma identidade; ela não depende de uma página limitada dos eventos mais recentes. Assim, uma carteira excepcional que acumule muitas decisões administrativas continua pertencendo ao mesmo grandfathering original e jamais ganha um novo período de 90 dias apenas porque o histórico ultrapassou um limite de leitura.

Os avisos são fatos idempotentes nos marcos início, D-60, D-30, D-15, D-7 e expiração. Cada horizonte temporal é avaliado a partir do instante em que passa a existir: uma extensão de 30 dias não retrocria D-60, e quando o início coincide com D-30 existe um único aviso canônico de início com 30 dias restantes. Cada payload contém limite contratado, ocupação, excesso, data final, bloqueio de novas coberturas e alternativas (`natural_endings`, `upgrade`, `admin_extension`).

O primeiro cruzamento também gera `professional_capacity_admin_alert_opened`. O ciclo do alerta continua sendo reavaliado enquanto a mesma janela existir: se a ocupação posteriormente ultrapassar a maior capacidade de plano profissional público ativo, um novo evento idempotente reabre a pendência como `catalog_range_review_required` com prioridade `high`; se a janela ou uma extensão vencer sem regularização, outro evento idempotente reabre a pendência para o horizonte vencido. Retries no mesmo range ou no mesmo `endsAt` não duplicam alertas, enquanto uma extensão posterior possui um novo horizonte e pode gerar uma nova reabertura no próprio vencimento. Nenhum desses alertas cria, publica ou precifica plano automaticamente.

Prorrogações são ações administrativas auditadas e finitas. Cada concessão registra uma identidade estável de decisão (`decisionId`), ator, motivo, estado da análise, limite temporário, início e fim de exatamente 30 dias. A mesma identidade de decisão é idempotente inclusive em retry ou entrega concorrente: ela devolve o horizonte já persistido e não acumula outros 30 dias. Um novo bloco de 30 dias exige uma nova identidade de decisão administrativa. Não existe extensão automática ou retroativa.
...
Os testes unitários cobrem limites temporais, cooldown de 12 meses, marcos de aviso, mudança de horizonte 90 -> 30 dias sem backfill temporal, estado de grandfathering, bloqueio de crescimento, escalonamento `catalog_range_review_required`, reabertura idempotente do alerta após cruzamento de range e por horizonte de vencimento, horizonte canônico de `past_due`, retry de perda clínica e ancoragem temporal em `contract_confirmed`. O controle temporal inclui falha antes do efeito, falha do receipt após o efeito e retry após restart, comprovando que nenhum desses caminhos troca `occurredAt` pelo relógio do worker. A integração TiDB parte de uma janela e alerta persistidos, cruza 45 -> 101 ocupações, repete retries, valida a identidade estável da decisão de extensão em retry sequencial e concorrente, atravessa mais de 20 extensões sem perder o `windowKey` original e avança pelos horizontes para provar tanto a reabertura por novo vencimento quanto a ausência de aviso D-60 impossível em uma extensão de 30 dias. O serviço também cobre o contrato de uma tentativa de cancelamento individual e o estado `pending` em falha remota.
