...
Quando todos os itens dependem de quantidade ou identidade, a pendência existente de detalhes alimentares preserva segmentos, índice do item pendente, causa semântica, data/timezone da refeição e resultados nutricionais dos itens já resolvidos; a gravação aguarda a resolução completa. Em um lote misto com pendência somente de medida, o gate remove apenas os segmentos quantitativamente pendentes do passthrough, registra os itens válidos em uma única refeição e informa explicitamente quais ficaram de fora. Uma identidade comercial não comprovada mantém o bloqueio atômico histórico. A retomada posterior de uma pendência continua usando a operação exata, sem duplicar os itens já registrados.
