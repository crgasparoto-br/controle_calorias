...
- **Ambiguidade por botões/lista**: `server/modules/whatsapp/mealItemSelectionCallback.ts` generaliza o padrão de seleção ambígua da exclusão (#782) para ajuste de gramas, correção de quantidade em contexto curto e substituição de alimento — persiste a pendência em `whatsappPendingOperations` (`type: "meal_item_selection"`), envia uma lista interativa com fallback textual numerado, e só aplica a mutação após a seleção (nunca antes). O callback de botão/lista é resolvido pelo mesmo `messageRouter.ts` central (`completeMealItemSelectionInteractiveCallback`); reentrega/clique duplo consome a pendência por compare-and-set, igual à exclusão. Integrado a `gramsAdjustmentIntent.ts`, `contextualFoodReplacementIntent.ts`, `intent/gramsAdjustmentHandlers.ts` e `intent/foodReplacementHandlers.ts` — os pontos de ambiguidade realmente alcançáveis pelo webhook de produção (`whatsappIntentWebhook.ts`).
- **Limitações conscientes, fora do escopo desta correção**: (1) quando `gramsAdjustmentHandlers.ts`/`foodReplacementHandlers.ts` não encontram o item na última refeição mas encontram múltiplos candidatos espalhados por refeições diferentes do mesmo dia, a resolução atual (`resolveTargetMealItemInMeals`) não carrega um `mealId` por candidato nessa combinação específica, então esse caso raro permanece com clarificação textual; (2) `recordAdjustmentIntent.ts` e `gramsIncrementIntent.ts` não estão no caminho do webhook real (usados apenas por `simulateWhatsappInbound`, o simulador web que retorna texto para o chamador em vez de enviar pela Cloud API), então não foram migrados — botões não teriam efeito nesse transporte.
