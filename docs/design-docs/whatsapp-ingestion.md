...
- Para os demais produtos industrializados reconhecidos por imagem, `brand`, variação e embalagem permanecem no item entregue a `createPendingMealInference`. A confirmação, a recarga pós-persistência e `buildWhatsAppMealReplyMessage` usam os mesmos nutrientes e o mesmo `foodName` comercial; a resposta não recompõe macros por outra referência.
