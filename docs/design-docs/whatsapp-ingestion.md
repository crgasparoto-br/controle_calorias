...
| `resolveStructuredCommercialIdentity` / `recoverCanonicalCommercialIdentity` | request contável | O owner resolve marca/variante somente a partir dos fatos estruturados; o segundo símbolo permanece apenas como adaptador de compatibilidade externa e não chama `processMealInput` | `commercialFoodIdentityPreflight::resolveStructuredCommercialIdentity` é owner; `recoverCanonicalCommercialIdentity` é bridge histórico sem consumidor produtivo interno | `keep` para compatibilidade externa; remover após varredura externa confirmar zero consumidores |
